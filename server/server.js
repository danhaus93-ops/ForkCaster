...
const LAB_KEYS = ["lipase","amylase","alt","ast","alb","trig","hdl","ldl","glucose","a1c","egfr","creat","tt","ft","shbg","e2","chol","hct","hgb","rbc","wbc","plt"];
app.post("/api/labs/parse", async (req, res) => {
  ...
  try {
    ...
    const system = "You extract laboratory values from a report. Return ONLY minified JSON, no prose, no markdown fences. " +
      ...
      "GLUCOSE->glucose, HEMOGLOBIN A1c->a1c, EGFR->egfr, CREATININE->creat, CHOLESTEROL TOTAL->chol, LIPASE->lipase, AMYLASE->amylase, " +
...
app.post("/api/report/pdf", async (req, res) => {
  try {
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><style>
      ...
      ${(() => { const d = fnd.doseResp || {}; const medName = esc((fnd.medication || {}).label || d.med || "current medication");
        if (d.switched) return `<tr><td><b>Medication change</b></td><td>Dose-response evidence restarted on ${esc(d.switchDate || "the switch")} for ${medName}; findings from the previous medication are held separately rather than combined.</td></tr>`;
        return ""; })()}
      ...
      ${fnd.contract ? `<tr><td style="width:140px"><b>Stated goal</b></td><td>${esc(fnd.contract.line)} Protein floor ${fnd.contract.proteinFloor} g/day (1 g per lb of lean mass); at a 0.65%/week loss rate that is roughly ${fnd.contract.weeksTarget} weeks.${fnd.contract.plausible ? "" : " NOTE: this goal implies a body-fat percentage below the healthy floor unless lean mass is lost."}</td></tr>` : ""}
