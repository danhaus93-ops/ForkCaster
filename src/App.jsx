...
export default function App() {
  ...
  const demoLink = (exId, name) => {
    ...
    return (<button onClick={() => openDemo(exId, name)} style={{ background: "none", border: "none", color: C.go, fontFamily: BODY, fontSize: 12.5, fontWeight: 700, cursor: "pointer", padding: "2px 0", textDecoration: "underline" }}>
  ...
  const photoPanes = (view, label, a, setA, b, setB, side, setSide, ref) => {
    ...
    return (
      <div style={{ marginTop: view === "back" ? 18 : 10 }}>
        <div style={{ fontSize: 12, letterSpacing: 1.2, color: C.muted, fontWeight: 800, marginBottom: 7 }}>{label.toUpperCase()}</div>
        {list.length === 0 ? (
          ...
          <>
              ...
              {[["Before", "A", a], ["After", "B", b]].map(([lbl, sd, idx]) => {
                ...
                return (
                  <div key={lbl} style={{ flex: 1 }}>
                    <button onClick={() => setSide(sd)} style={{ fontSize: 12, marginBottom: 5, fontWeight: 700, background: "none", border: "none", cursor: "pointer", color: side === sd ? C.go : C.muted, padding: 0 }}>{side === sd ? "● " : ""}{lbl}</button>
                    <div style={{ position: "relative", aspectRatio: "3/4", borderRadius: 12, overflow: "hidden", background: C.surfaceAlt, border: `1.5px solid ${side === sd ? C.go : C.hair}` }}>
                      ...
                      {shown && <button onClick={() => deletePhoto(idx)} style={{ position: "absolute", top: 6, right: 6, width: 26, height: 26, borderRadius: 13, background: "rgba(14,20,26,0.72)", color: "#fff", border: "none", fontSize: 14, cursor: "pointer" }}>✕</button>}
                    </div>
                    <div style={{ fontSize: 11.5, color: C.faint, marginTop: 4, textAlign: "center" }}>{shown ? fmtDate(shown.date) : ""}{w ? ` · ${fmtWt(w.lbs)} ${wtU}` : ""}</div>
                  </div>
                );
              })}
            </div>
            ...
            <div style={{ fontSize: 11, color: C.faint, marginTop: 2 }}>Tap a pane label to choose which side the strip sets · purple = Before, green = After</div>
            {list.length >= 2 && a !== b && inView(a) && inView(b) && <button onClick={() => shareComparison(a, b)} style={{ width: "100%", marginTop: 10, background: C.violet, color: "#fff", border: "none", borderRadius: 11, padding: "12px 0", fontFamily: BODY, fontSize: 14.5, fontWeight: 700, cursor: "pointer" }}>Share this {label.toLowerCase()} comparison →</button>}
          </>
        )}
        <input ref={ref} type="file" accept="image/*" multiple onChange={(e) => addPhotos(e, view)} style={{ display: "none" }} />
        <button onClick={() => ref.current && ref.current.click()} style={{ width: "100%", marginTop: 8, background: C.surfaceAlt, color: C.ink, border: `1px dashed ${C.faint}`, borderRadius: 11, padding: "11px 0", fontFamily: BODY, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>+ Add {label.toLowerCase()} photo</button>
      </div>
    );
  };
  ...
  const linkBtn = { marginTop: 12, background: "none", border: "none", color: C.muted, fontSize: 13.5, fontFamily: BODY, cursor: "pointer", textDecoration: "underline", padding: 0 };
  const chipBtn = { background: C.surfaceAlt, border: `1px solid ${C.hair}`, borderRadius: 20, padding: "7px 13px", fontSize: 13.5, fontWeight: 600, color: C.ink, cursor: "pointer", fontFamily: BODY };
  const arrowBtn = { background: C.surfaceAlt, border: `1px solid ${C.hair}`, borderRadius: 8, width: 30, height: 26, fontSize: 16, color: C.ink, cursor: "pointer" };
  const selectStyle = { flex: 1, fontFamily: BODY, fontSize: 14.5, color: C.ink, background: C.surfaceAlt, border: `1px solid ${C.hair}`, borderRadius: 10, padding: "10px 11px", outline: "none" };
    ...
    return cardShell(
      <div onClick={() => setSheetCard({ id: vd.id, title: vd.title, color: vd.color, when: vd.when })} style={{ cursor: "pointer" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 8 }}>
          ...
          {vd.when && <span style={{ marginLeft: "auto", fontFamily: DATA, fontSize: 10, color: C.faint, letterSpacing: 0.5, textTransform: "uppercase", flexShrink: 0 }}>{vd.when}</span>}
          <span style={{ color: C.faint, fontSize: 15, lineHeight: 1, marginLeft: vd.when ? 6 : "auto", flexShrink: 0 }}>{"›"}</span>
        </div>
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 10 }}>
          <div style={{ minWidth: 0 }}>
            <span style={{ fontFamily: DATA, fontSize: 26, fontWeight: 700, letterSpacing: -0.5, lineHeight: 1, color: C.ink }}>{vd.value}</span>
            {vd.unit && <span style={{ fontFamily: DATA, fontSize: 11, color: C.faint, marginLeft: 4, fontWeight: 600 }}>{vd.unit}</span>}
            {vd.sub && <div style={{ fontFamily: DATA, fontSize: 9.5, color: vd.subTone || C.faint, marginTop: 4, letterSpacing: 0.3, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{vd.sub}</div>}
          </div>
  ...
  const stat = (label, value, unit, color = C.ink) => (
    <div style={{ flex: 1 }}>
      <div style={{ fontFamily: DATA, fontSize: 10.5, fontWeight: 700, color: C.muted, letterSpacing: 1.3, textTransform: "uppercase" }}>{label}</div>
      <div style={{ fontFamily: DATA, fontSize: 23, fontWeight: 700, color, fontVariantNumeric: "tabular-nums", lineHeight: 1.1 }}>{value}<span style={{ fontSize: 13, color: C.faint, fontWeight: 500 }}>{unit}</span></div>
    </div>
  );
  ...
  const quickRow = () => quick && (
    <div style={{ display: "flex", gap: 8, alignItems: "center", marginTop: 10, background: C.surfaceAlt, border: `1px solid ${C.hair}`, borderRadius: 12, padding: "10px 12px" }}>
      <span style={{ fontSize: 13.5, color: C.muted, whiteSpace: "nowrap" }}>add {quick.label}</span>
      <input autoFocus type="number" inputMode="decimal" value={quickVal} onChange={(e) => setQuickVal(e.target.value)}
        ...
        placeholder={quick.unit} style={{ flex: 1, minWidth: 0, fontFamily: BODY, fontSize: 16, color: C.ink, background: C.surface, border: `1px solid ${C.hair}`, borderRadius: 9, padding: "9px 11px", outline: "none" }} />
      <button onClick={() => commitQuick("add")} style={{ background: C.go, color: C.surface, border: "none", borderRadius: 9, padding: "9px 15px", fontFamily: BODY, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>Add</button>
      <button onClick={() => commitQuick("set")} title="Replace today's total for this number with the value typed (blank = 0)" style={{ background: "none", border: `1.5px solid ${C.hair}`, color: C.ink, borderRadius: 9, padding: "9px 13px", fontFamily: BODY, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>Set</button>
      <button onClick={() => { setQuick(null); setQuickVal(""); }} style={{ background: "none", border: "none", color: C.faint, fontSize: 18, cursor: "pointer", padding: "0 2px" }}>✕</button>
    </div>
  );
  const renderNow = () => (
    <div style={{ padding: "18px 18px 12px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <button onClick={() => detectLocation(true)} style={{ background: "none", border: "none", padding: 0, cursor: "pointer", textAlign: "left", fontSize: 12, letterSpacing: 1.1, color: C.muted, textTransform: "uppercase", fontWeight: 600, fontFamily: BODY }}>{geoLabel(geo, timeStr)}</button>
        <div style={{ fontSize: 12, color: C.go, fontWeight: 600, display: "flex", alignItems: "center", gap: 5 }}><span style={{ width: 7, height: 7, borderRadius: 99, background: C.go }} /> {MODES[mode].label}</div>
      </div>
      ...
      {(() => { const k = dayKeyAt(Date.now(), prefs); const plain = new Date().toLocaleDateString("sv-SE");
        ...
        return (<div onClick={() => setSettingsOpen(true)} style={{ marginTop: 10, display: "flex", alignItems: "center", gap: 7, flexWrap: "wrap", cursor: "pointer" }}>
          <span style={{ fontFamily: DATA, fontSize: 9.5, fontWeight: 700, letterSpacing: 1, borderRadius: 999, padding: "3px 9px", color: night ? C.violet : C.faint, border: `1px solid ${night ? C.violet + "55" : C.hair}`, background: night ? C.violet + "14" : "transparent", textTransform: "uppercase" }}>{night ? "Night day" : "Day"} · counts toward {new Date(k + "T12:00:00").toLocaleDateString([], { weekday: "short", month: "short", day: "numeric" })}</span>
          <span style={{ fontSize: 11.5, color: C.faint }}>resets {h12(rollAt)}</span>
        </div>); })()}

      <div style={{ marginTop: 16, display: "flex", alignItems: "baseline", gap: 10 }}>
        <div onClick={() => openQuick("protein", "protein", "g")} style={{ fontFamily: DISPLAY, fontSize: 55, fontWeight: 700, color: C.ink, lineHeight: 0.9, fontVariantNumeric: "tabular-nums", cursor: "pointer" }}>{proteinLeft}<span style={{ fontSize: 23, color: C.muted }}>g</span></div>
        <div style={{ paddingBottom: 6 }}><div onClick={() => openQuick("protein", "protein", "g")} style={{ fontSize: 16, fontWeight: 600, color: C.ink, cursor: "pointer" }}>protein to go</div><div onClick={() => openQuick("calories", "calories", "cal")} style={{ fontSize: 14, color: C.muted, cursor: "pointer" }}>{calLeft} cal left today</div></div>
      </div>
      ...
      <div style={{ display: "flex", gap: 6, marginTop: 16 }}>
        {[
          ...
          <div key={m.label} onClick={() => openQuick(m.field, m.short, m.unit)} style={{ flex: 1, minWidth: 0, textAlign: "center", cursor: "pointer" }}>
            ...
            <div style={{ fontFamily: DATA, fontSize: 9.5, fontWeight: 700, letterSpacing: 0.4, textTransform: "uppercase", color: m.col, marginTop: 2, lineHeight: 1.3 }}>{m.label}</div>
          </div>); })}
      </div>

      {quickRow()}
      <div style={{ fontSize: 12, color: C.faint, marginTop: 8, textAlign: "center" }}>Tap any number above to log intake by hand</div>
      ...
      {editing && card(
        <>
          ...
          <button onClick={() => { setPrefs({ ...prefs, customTargets: { ...targets } }); setMode("custom"); setPresetSaved(true); setTimeout(() => setPresetSaved(false), 2200); }} style={{ marginTop: 12, width: "100%", background: "none", border: `1.5px solid ${C.go}`, color: C.go, borderRadius: 10, padding: "10px 0", fontFamily: BODY, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>{presetSaved ? "Preset saved ✓" : "Save these as \"My preset\""}</button>
        </>, { marginTop: 10 })}
      ...
      {onMed && nauseaRisk !== "low" && (
        <div style={{ marginTop: 16, background: C.violet + "14", border: `1px solid ${C.violet}44`, borderRadius: 14, padding: "12px 14px", display: "flex", gap: 11 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 13.5, color: C.ink2, marginTop: 3, lineHeight: 1.4 }}>
        ...
        <div style={{ marginTop: 14, background: C.avoidSoft, border: `1px solid ${C.avoid}40`, borderRadius: 14, padding: "11px 14px", display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <div style={{ fontSize: 13.5, color: C.ink2, lineHeight: 1.4 }}>
            ...
            <span style={{ display: "inline-flex", flexWrap: "wrap", gap: 5, margin: "0 0 0 5px", verticalAlign: "middle" }}>
              {allergies.map((a) => (
                <span key={a} onClick={() => setAllergies((xs) => xs.filter((y) => y !== a))} style={{ fontFamily: DATA, fontSize: 10, fontWeight: 700, borderRadius: 999, padding: "3px 8px", color: C.avoid, border: `1px solid ${C.avoid}44`, cursor: "pointer" }}>{a} ✕</span>))}
              {diets.map((d) => (
                <span key={d} style={{ fontFamily: DATA, fontSize: 10, fontWeight: 700, borderRadius: 999, padding: "3px 8px", color: C.avoid, border: `1px solid ${C.avoid}44` }}>{d}</span>))}
            </span>
      ...
      <div style={{ marginTop: 18 }}>
        ...
        {geo.status === "ok" && venues.length === 0 && (
          <div style={{ background: C.surface, border: `1px solid ${C.hair}`, borderRadius: 16, padding: "22px 18px", textAlign: "center", marginBottom: 6 }}>
            <div style={{ fontSize: 16, fontWeight: 700, color: C.ink }}>No food spots near you</div>
            <div style={{ fontSize: 13.5, color: C.muted, marginTop: 5, lineHeight: 1.5 }}>Nothing within ~2 miles of this point. Keep driving, or drag the map ahead and tap "Search this area" to scout your route.</div>
          </div>
        )}
            ...
            return (
              <div key={r.id} style={{ minWidth: 178, background: C.surface, borderRadius: 16, flexShrink: 0, overflow: "hidden", border: `1.5px solid ${active ? C.go : C.hair}`, boxShadow: active ? `0 6px 18px ${C.go}22` : `0 1px 3px ${C.ink}0A` }}>
                <div style={{ height: 100, position: "relative", overflow: "hidden" }}>
                  ...
                  {r.menu && <div style={{ position: "absolute", top: 8, left: 8, background: "rgba(200,140,20,0.95)", color: "#1a1200", borderRadius: 20, padding: "3px 9px", fontSize: 10, fontWeight: 800, letterSpacing: 0.5 }}>DEMO</div>}
                  <div style={{ position: "absolute", top: 8, right: 8, background: "rgba(255,255,255,0.95)", borderRadius: 20, padding: "3px 9px", display: "flex", alignItems: "center", gap: 3, boxShadow: "0 1px 4px rgba(0,0,0,0.15)" }}>
                    <span style={{ fontFamily: DISPLAY, fontWeight: 700, fontSize: 14, color: r.match != null ? scoreColor(r.match / 20) : r.menu ? sc : "#6b7a71" }}>{r.match != null ? r.match : r.menu ? Math.round(r.score * 20) : r.score.toFixed(1)}</span>
                    <span style={{ fontSize: 10, fontWeight: 700, color: r.match != null ? scoreColor(r.match / 20) : r.menu ? sc : "#6b7a71", textTransform: "uppercase" }}>{r.match != null || r.menu ? "match" : "★"}</span>
                  </div>
                  <div style={{ position: "absolute", left: 12, bottom: 9 }}>
                    ...
                    <div style={{ fontSize: 12, color: "rgba(255,255,255,0.92)", marginTop: 2, textShadow: "0 1px 2px rgba(0,0,0,0.4)" }}>{r.cuisine} · {r.lat != null && geo.status === "ok" ? `${distMi(geo.lat, geo.lng, r.lat, r.lng).toFixed(1)} mi` : r.eta}</div>
                    {r.why && <div style={{ fontSize: 11, color: "rgba(255,255,255,0.85)", marginTop: 1, textShadow: "0 1px 2px rgba(0,0,0,0.4)" }}>{r.why}</div>}
                  </div>
                </div>
                <div style={{ padding: 12 }}>
                  <button onClick={() => orderForMe(r)} disabled={loading && active} style={{ width: "100%", background: active ? C.go : "transparent", color: active ? C.bg : C.go, border: active ? "none" : `1px solid ${C.go}55`, borderRadius: 999, padding: "11px 0", fontFamily: DATA, fontSize: 12.5, fontWeight: 800, letterSpacing: 1.2, textTransform: "uppercase", cursor: "pointer", opacity: loading && active ? 0.6 : 1 }}>{loading && active ? "Thinking…" : "Order for me"}</button>
                </div>
              </div>
            );
          })}
      ...
      <div style={{ marginTop: 6 }}>
        {error && <div style={{ background: C.avoidSoft, color: C.avoid, borderRadius: 12, padding: 14, fontSize: 14.5, marginTop: 12 }}>{error}</div>}
        {loading && !result && <div style={{ textAlign: "center", color: C.muted, fontSize: 14.5, padding: "22px 0" }}>Reading the menu against your {proteinLeft}g / {calLeft} cal…</div>}
        {result && result._menuSource && (<>
                <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: 0.4, color: result._menuSource === "live" ? C.go : C.faint, marginBottom: 8 }}>
          ...
          <div style={{ marginTop: 14 }}>
            {result.coachLine && (
              <div style={{ background: C.goSoft, border: `1px solid ${C.go}33`, borderRadius: 14, padding: "13px 15px", marginBottom: 14 }}>
                <div style={{ fontSize: 11.5, color: C.go, fontWeight: 700, letterSpacing: 0.6, textTransform: "uppercase", marginBottom: 3 }}>Coach</div>
                <div style={{ fontSize: 15.5, color: C.ink2, fontWeight: 500, lineHeight: 1.35 }}>{result.coachLine}</div>
              </div>
            )}
            {(result.picks || []).map((p, i) => (
              <div key={i} style={{ display: "flex", gap: 12, alignItems: "center", background: C.surface, marginBottom: 10, border: `1px solid ${C.hair}`, borderLeft: `4px solid ${medalColor(i)}`, borderRadius: 14, padding: "12px 14px" }}>
                <div style={{ width: 42, height: 42, borderRadius: 11, flexShrink: 0, overflow: "hidden", position: "relative", boxShadow: `0 1px 3px ${C.ink}22` }}>
                  ...
                  <div style={{ position: "absolute", top: -5, left: -5, width: 18, height: 18, borderRadius: 99, background: medalColor(i), color: "#fff", fontSize: 12, fontWeight: 700, fontFamily: DISPLAY, display: "flex", alignItems: "center", justifyContent: "center", border: "1.5px solid #fff", zIndex: 2 }}>{i + 1}</div>
                </div>
                <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 15.5, fontWeight: 600, color: C.ink, lineHeight: 1.2 }}>{p.name}</div><div style={{ fontSize: 13.5, color: C.muted, marginTop: 2, lineHeight: 1.45 }}>{p.why ? <><b style={{ color: C.go, fontWeight: 700 }}>Why:</b> {p.why}</> : null}</div></div>
                <div style={{ textAlign: "right", flexShrink: 0 }}><div style={{ fontFamily: DISPLAY, fontSize: 20, fontWeight: 700, color: C.go, fontVariantNumeric: "tabular-nums" }}>{p.protein}g</div><div style={{ fontSize: 12.5, color: C.faint }}>{(p.calories ?? p.cal) || "—"} cal{p.carbs != null ? ` · ${p.carbs}g carb` : ""}{p.fat != null ? ` · ${p.fat}g fat` : ""}</div></div>
                <button onClick={() => { const nm = p.item || p.name; if (loggedPicks.includes(nm)) return; const pr = +p.protein || 0, ca = +(p.calories ?? p.cal) || 0, fa = p.fat == null ? 0 : +p.fat || 0, cb = p.carbs == null ? 0 : +p.carbs || 0, fb = +p.fiber || 0; setEaten((e) => ({ ...e, protein: e.protein + pr, calories: e.calories + ca, carbs: (e.carbs || 0) + cb, fat: (e.fat || 0) + fa, fiber: (e.fiber || 0) + fb })); setMealLog((m) => [...m, { id: uid(), date: todayISO(), name: nm, protein: pr, calories: ca, fat: fa, carbs: cb, fiber: fb }]); setLoggedPicks((l) => [...l, nm]); }}
                  style={{ marginLeft: 10, flexShrink: 0, alignSelf: "center", background: loggedPicks.includes(p.item || p.name) ? C.goSoft : "none", border: `1.5px solid ${C.go}`, color: C.go, borderRadius: 9, padding: "7px 10px", fontFamily: BODY, fontSize: 12.5, fontWeight: 800, cursor: "pointer" }}>{loggedPicks.includes(p.item || p.name) ? "✓" : "Log"}</button>
              </div>
            ))}
            {result.picks && result.picks[0] && (
              <>
                {(() => {
                  ...
                  return (
                    <>
                      <div style={{ marginTop: 10, fontSize: 13.5, color: C.muted, fontFamily: BODY }}>Order "{_item}" from</div>
                      ...
                      <div style={{ textAlign: "center", fontSize: 12, color: C.faint, marginTop: 7 }}>Opens the app at {_venue} and copies the item name — paste it in their search</div>
                    </>
                  );
                })()}
              </>
            ...
            {(result.avoid || []).length > 0 && (
              <div style={{ marginTop: 18 }}>
                ...
                <div style={{ fontSize: 11.5, color: C.faint, marginTop: -4, marginBottom: 8, lineHeight: 1.45 }}>From your fat ceiling and today's symptom load.</div>
                {result.avoid.map((a, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", gap: 10, padding: "8px 0", borderBottom: i < result.avoid.length - 1 ? `1px solid ${C.hair}` : "none" }}>
                    <span style={{ fontSize: 14.5, color: C.ink, fontWeight: 500 }}>{a.item || a.name}</span><span style={{ fontSize: 13.5, color: C.muted, textAlign: "right", maxWidth: "48%" }}>{a.reason}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            <div style={{ marginTop: 14 }}>
              <div style={{ display: "flex", gap: 8 }}>
                <input value={menuQ} onChange={(e) => setMenuQ(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") askMenu(); }} placeholder="Ask about this menu… (swaps, carbs, drinks)"
                  style={{ flex: 1, fontFamily: BODY, fontSize: 14.5, color: C.ink, background: C.surfaceAlt, border: `1px solid ${C.hair}`, borderRadius: 11, padding: "11px 13px", outline: "none" }} />
                <button onClick={askMenu} disabled={menuQBusy} style={{ background: C.go, color: C.bg, border: "none", borderRadius: 999, padding: "0 16px", fontFamily: DATA, fontSize: 12, fontWeight: 800, letterSpacing: 1, textTransform: "uppercase", cursor: "pointer", opacity: menuQBusy ? 0.6 : 1 }}>{menuQBusy ? "…" : "Ask"}</button>
              </div>
              {menuA && <div style={{ marginTop: 8, fontSize: 14, color: C.ink2, lineHeight: 1.5, background: C.surfaceAlt, border: `1px solid ${C.hair}`, borderRadius: 11, padding: "11px 13px" }}>{menuA}</div>}
            </div>
          </div>
        )}
      </div>
  ...
  const renderToday = () => {
    ...
    return (
      <div style={{ padding: "18px 18px 12px" }}>
        <div style={{ fontFamily: DISPLAY, fontSize: 25, fontWeight: 700, color: C.ink }}>Today</div>
        <div style={{ fontSize: 14, color: C.muted, marginBottom: 14 }}>{new Date().toLocaleDateString([], { weekday: "long", month: "long", day: "numeric" })}{(() => { const k = todayISO(), plain = dayKeyAt(Date.now(), { rolloverHour: prefs.rolloverHour }); return k !== plain ? <span style={{ marginLeft: 8, fontSize: 12, fontWeight: 800, color: C.violet, border: `1px solid ${C.violet}55`, borderRadius: 20, padding: "3px 9px" }}>NIGHT DAY · counts toward {new Date(k + "T12:00:00").toLocaleDateString([], { weekday: "short" })}</span> : null; })()}</div>
        ...
        <div style={{ marginBottom: 10 }}>{card(<>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            ...
            <span style={{ fontFamily: DATA, fontSize: 9.5, fontWeight: 700, letterSpacing: 1, borderRadius: 999, padding: "3px 9px", marginTop: -8, color: pTone, border: `1px solid ${pTone}55`, background: pTone + "1A" }}>{pPct}% OF FLOOR</span>
          </div>
          <div style={{ display: "flex", alignItems: "baseline", gap: 7 }}>
            <span style={{ width: 6, height: 6, borderRadius: 6, background: C.go, alignSelf: "center" }} />
            <span style={{ fontFamily: DATA, fontSize: 39, fontWeight: 700, color: C.ink, lineHeight: 1 }}>{eaten.protein || 0}</span>
            <span style={{ fontFamily: DATA, fontSize: 14, color: C.muted }}>/ {targets.protein}g</span>
          </div>
          ...
          <div style={{ fontSize: 12.5, color: C.muted, marginTop: 9, lineHeight: 1.5 }}>{pGap > 0 ? `${pGap}g short. A shake or a palm of chicken closes most of it.` : "Floor met. This is the day that protects the muscle."}</div>
        </>, { borderLeft: `2.5px solid ${pTone}` })}</div>

        <div style={{ marginBottom: 10 }}>{card(<>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 2 }}>
            {sectionTitle("Counters", C.muted)}
            <span onClick={() => openQuick("calories", "Calories", "")} style={{ fontFamily: DATA, fontSize: 11, fontWeight: 700, letterSpacing: 1.2, textTransform: "uppercase", color: C.go, cursor: "pointer", marginTop: -8 }}>Set / add</span>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            {counters.map(([l, v, sub, fld]) => (<div key={l} onClick={() => fld && openQuick(fld, l, "")} style={{ flex: 1, minWidth: 0, cursor: fld ? "pointer" : "default" }}>
              <div style={{ fontFamily: DATA, fontSize: 10, letterSpacing: 1.1, color: C.faint, textTransform: "uppercase" }}>{l}</div>
              <div style={{ fontFamily: DATA, fontSize: 15.5, fontWeight: 700, color: C.ink, marginTop: 3, fontVariantNumeric: "tabular-nums" }}>{v}</div>
              <div style={{ fontFamily: DATA, fontSize: 10, color: C.faint, marginTop: 1 }}>{sub}</div>
            </div>))}
          </div>
        ...
        <div style={{ marginBottom: 10 }} onClick={() => { setScan({ status: "idle" }); setBarcode(""); setLogOpen(true); }}>{card(<>
          ...
          {[["Barcode scan", "Most exact", C.go], ["Nutrition label photo", "Exact", C.go], ["Search USDA / Open Food Facts", "Good", C.muted], ["Photo estimate — snap your plate", "Estimate", C.caution], ["Describe it — AI estimate", "Least exact", C.caution]].map(([l, tier, tone], i) => (
            <div key={l} onClick={(ev) => { if (i === 1) { ev.stopPropagation(); labelRef.current && labelRef.current.click(); return; } setScan({ status: "idle" }); setBarcode(""); setLogOpen(true); }} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, padding: "9px 0", borderTop: i ? `1px solid ${C.hair}` : "none", cursor: "pointer" }}>
              ...
              <span style={{ fontFamily: DATA, fontSize: 10, fontWeight: 700, letterSpacing: 1, borderRadius: 999, padding: "3px 8px", color: tone, border: `1px solid ${tone}55`, whiteSpace: "nowrap", textTransform: "uppercase" }}>{tier}</span>
            </div>))}
          <div style={{ fontSize: 12, color: C.faint, marginTop: 9, lineHeight: 1.5 }}>Trends beat single meals. Snap labels when you can.</div>
        </>)}</div>
        ...
        {mealLog.filter((m) => m.date === todayISO()).length > 0 && <div style={{ marginBottom: 10 }}>{card(<>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
            ...
            <span onClick={() => openQuick("calories", "Calories", "")} style={{ fontFamily: DATA, fontSize: 11, fontWeight: 700, letterSpacing: 1.2, textTransform: "uppercase", color: C.go, cursor: "pointer", marginTop: -8 }}>+ Quick add</span>
          </div>
          {mealLog.filter((m) => m.date === todayISO()).map((m) => (
            <div key={m.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: `1px solid ${C.hair}` }}>
              <div style={{ minWidth: 0, paddingRight: 8 }}><div style={{ fontSize: 14.5, fontWeight: 600, color: C.ink, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{m.name}</div>
                <div style={{ fontFamily: DATA, fontSize: 11.5, color: C.faint, marginTop: 2 }}>{[m.at ? new Date(m.at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : null, `${m.protein} P`, m.src ? String(m.src).toLowerCase().replace(/ \(.*\)$/, "") : null].filter(Boolean).join(" · ")}</div></div>
              <div style={{ display: "flex", alignItems: "center", gap: 9, flexShrink: 0 }}><span style={{ fontFamily: DATA, fontSize: 14, fontWeight: 700, color: C.ink2 }}>{m.calories}</span>
              <button onClick={() => { setEaten((e) => ({ ...e, protein: Math.max(0, e.protein - (m.protein || 0)), calories: Math.max(0, e.calories - (m.calories || 0)), carbs: Math.max(0, (e.carbs || 0) - (m.carbs || 0)), fat: Math.max(0, (e.fat || 0) - (m.fat || 0)), fiber: Math.max(0, (e.fiber || 0) - (m.fiber || 0)) })); setMealLog((l) => l.filter((x) => x.id !== m.id)); }} style={{ background: "none", border: "none", color: C.faint, fontSize: 16, cursor: "pointer", padding: 4, flexShrink: 0 }}>✕</button></div>
            </div>
          ))}
        </>)}</div>}
          ...
          <div style={{ display: "flex", gap: 5 }}>
            {wk7.map((d, i) => (
              <div key={i} style={{ flex: 1, textAlign: "center", borderRadius: 10, padding: "7px 0 6px", background: d.dose ? C.violet + "1A" : d.after ? C.violet + "0D" : "transparent", border: `1px solid ${d.dose ? C.violet + "55" : d.after ? C.violet + "22" : C.hair}` }}>
                <div style={{ fontFamily: DATA, fontSize: 9.5, fontWeight: 700, letterSpacing: 0.2, color: d.dose || d.after ? C.violet : (i === 0 ? C.ink : C.faint), textTransform: "uppercase" }}>{d.label}</div>
                <div style={{ fontSize: 11, marginTop: 3, color: C.violet, lineHeight: 1 }}>{d.dose ? "💉" : d.after ? "·" : " "}</div>
              </div>))}
          </div>
          <div style={{ fontSize: 12.5, color: C.muted, marginTop: 10, lineHeight: 1.5 }}>{wk7[0] && wk7[0].dose ? "Shot day — lighter, low-fat and carb-forward keeps it down." : wk7[0] && wk7[0].after ? "Day after the shot — targets ease; eat slowly and lead with protein." : doseDay ? `Next shot lands on ${doseDayLong}. Targets ease that day and the one after.` : "Set your injection day on the GLP-1 tab and the week syncs to it."}</div>
        </>)}</div>

        <div>{card(<>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            {sectionTitle("Activity", C.muted)}
            <span style={{ fontFamily: DATA, fontSize: 9.5, fontWeight: 700, letterSpacing: 1, borderRadius: 999, padding: "3px 9px", marginTop: -8, color: syn > 0 ? C.go : C.faint, border: `1px solid ${syn > 0 ? C.go + "55" : C.hair}`, background: syn > 0 ? C.go + "1A" : "transparent" }}>{syn > 0 ? (prefs.stepSource || "SYNCED") : "NOT SYNCED"}</span>
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            {[["Steps", stepsShown ? stepsShown.toLocaleString() : null, syn > 0, syn > (+eaten.steps || 0) ? "synced \u00b7 goal 10,000" : "goal 10,000"],
              ...
              <div key={l} style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontFamily: DATA, fontSize: 10, letterSpacing: 1.1, color: C.faint, textTransform: "uppercase" }}>{l}</div>
                <div style={{ fontFamily: DATA, fontSize: 16, fontWeight: 700, color: v ? C.ink : C.faint, marginTop: 3, display: "flex", alignItems: "center", gap: 5 }}>
  ...
  const renderTrain = () => {
    ...
    const nav = (
      <div style={{ display: "flex", gap: 7, margin: "0 0 12px", flexWrap: "wrap" }}>
        {[["today", "Today"], ["week", "Week"], ["lifts", "Lifts"], ["setup", "Routine"]].map(([k, l]) => (
          <button key={k} onClick={() => setTrainView(k)} style={{ background: trainView === k ? C.go + "22" : "transparent", color: trainView === k ? C.go : C.muted, border: `1.5px solid ${trainView === k ? C.go : C.hair}`, borderRadius: 999, padding: "8px 15px", fontFamily: DATA, fontSize: 12, fontWeight: 700, letterSpacing: 1.1, textTransform: "uppercase", cursor: "pointer" }}>{l}</button>
        ))}
      </div>
    );
    if (trainView === "session" && liveSession) return (
      <div style={{ padding: "14px 18px" }}>
        <button onClick={() => { setLiveSession(null); setTrainView("today"); }} style={{ background: "none", border: "none", color: C.go, fontFamily: BODY, fontSize: 14.5, fontWeight: 700, cursor: "pointer", padding: 0, marginBottom: 10 }}>← Cancel session</button>
        <div style={{ fontFamily: DISPLAY, fontSize: 25, fontWeight: 700, color: C.ink, marginBottom: 12 }}>{liveSession.name}</div>
        {(() => {   // a core day runs as a circuit — say so once, at the top, instead of per-exercise
          ...
          return (
            <div style={{ marginBottom: 12, background: C.surfaceAlt, border: `1px solid ${C.hair}`, borderRadius: 12, padding: "10px 13px" }}>
              <div style={{ fontSize: 13.5, fontWeight: 700, color: C.ink, fontFamily: BODY }}>Core runs as a circuit</div>
        ...
        {(() => { const first = liveSession.entries.find((e) => e.mechanic === "compound") || liveSession.entries[0];
          ...
          return card(<div>
            ...
            {first && <div style={{ fontSize: 13.5, color: C.ink, lineHeight: 1.5, marginBottom: dyn.length ? 8 : 0 }}>
              <b>Ramp into {first.name}:</b> {ramp.map((r) => (r.w ? `${r.w} lb × ${r.reps}` : r.label)).join(" → ")} <span style={{ color: C.faint }}>· then your working sets</span>
            </div>}
            {dyn.map((x, i) => <div key={i} style={{ fontSize: 13.5, color: C.muted, padding: "3px 0" }}>{x.name} <span style={{ color: C.faint }}>· {x.prescribe}</span></div>)}
          </div>, { marginBottom: 10 }); })()}
        {liveSession.entries.map((e, ei) => (<div key={ei}>
          {e.group === "core" && (ei === 0 || liveSession.entries[ei - 1].group !== "core") && (
            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 8, margin: "16px 0 8px" }}>
              <div style={{ fontSize: 12, letterSpacing: 1.6, color: C.muted, fontWeight: 800 }}>CORE · timed circuit</div>
              <a href={sixpackUrl()} rel="noopener" onClick={() => { const t = setTimeout(() => { if (!document.hidden) window.location.href = SIXPACK.store; }, sixpackFallbackDelay(document.hidden)); const stop = () => clearTimeout(t); document.addEventListener("visibilitychange", stop, { once: true }); window.addEventListener("pagehide", stop, { once: true }); }} style={{ fontSize: 12, color: C.violet, fontWeight: 700, textDecoration: "none", whiteSpace: "nowrap" }}>or use {SIXPACK.label} →</a>
            </div>
          )}
          {card(<div>
          <div style={{ fontSize: 15.5, fontWeight: 800, color: C.ink }}>{e.name}</div>
          <div style={{ fontSize: 12.5, color: C.muted, margin: "2px 0 3px" }}>{e.group} · {e.equipment} · target {prescription(e)} · rest {e.restSec}s</div>
          ...
          {e.sets.map((st, si) => (
            <div key={si} style={{ display: "flex", gap: 7, alignItems: "center", marginBottom: 6 }}>
              <div style={{ fontSize: 12.5, color: C.faint, width: 34 }}>set {si + 1}</div>
        ...
        {(() => { const cd = cooldownBlock(exCatalog, [...new Set(liveSession.entries.map((e) => e.group))]);
          return cd.length ? card(<div>
            ...
            {cd.map((x, i) => <div key={i} style={{ fontSize: 13.5, color: C.muted, padding: "3px 0" }}>{x.name} <span style={{ color: C.faint }}>· {x.prescribe}</span></div>)}
          </div>, { marginBottom: 10 }) : null; })()}
        <button onClick={saveSession} style={{ width: "100%", background: C.go, color: C.surface, border: "none", borderRadius: 12, padding: "14px 0", fontFamily: BODY, fontSize: 16, fontWeight: 800, cursor: "pointer" }}>Finish &amp; log session ✓</button>
        {restEnd && (
          <div style={{ position: "fixed", left: 12, right: 12, bottom: "calc(72px + env(safe-area-inset-bottom, 0px))", zIndex: 300, background: C.surfaceAlt, color: C.ink, border: `1px solid ${C.go}55`, borderRadius: 14, padding: "12px 14px", display: "flex", alignItems: "center", gap: 10, boxShadow: "0 8px 24px rgba(0,0,0,.28)" }}>
            <div style={{ fontFamily: DISPLAY, fontSize: 23, fontWeight: 700, fontVariantNumeric: "tabular-nums" }}>{fmtRest(restLeft)}</div>
    ...
    return (
      <div style={{ padding: "14px 18px" }}>
        <div style={{ fontFamily: DISPLAY, fontSize: 25, fontWeight: 700, color: C.ink }}>Train</div>
        <div style={{ fontSize: 14, color: C.muted, marginBottom: 12 }}>Lifting is the half of muscle protection food can't do</div>
        {nav}
        {trainView === "today" && (!routine ? card(<div style={{ fontSize: 14, color: C.muted, lineHeight: 1.55 }}>No routine yet — open <b style={{ color: C.ink }}>Routine</b> and generate one. Pick your days per week, equipment, and session length; everything else is automatic.</div>) : (<div>
          {(() => { const rd = readinessRead((healthSync && healthSync.days) || [], glp.doseLog, workoutLog, rhrRead((healthSync && healthSync.days) || [], glp.doseLog), sleepRead((healthSync && healthSync.days) || [], glp.doseLog), todayISO());
            ...
            return (<div style={{ marginBottom: 10 }}>{card(<>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <span style={{ fontFamily: DATA, fontSize: 9.5, fontWeight: 700, letterSpacing: 1, borderRadius: 999, padding: "3px 9px", marginTop: -8, color: BC, border: `1px solid ${BC}55`, background: BC + "1A" }}>{rd.band}</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 10 }}>
                ...
                <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px 12px" }}>
                  {["RHR", "Sleep", "Post-dose", "Load"].map((slot) => {
                    ...
                    return (<div key={slot}>
                      <div style={{ fontFamily: DATA, fontSize: 10, letterSpacing: 1.1, color: C.faint, textTransform: "uppercase" }}>{slot}</div>
                      {pt ? (
              ...
              <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.5, borderTop: `1px solid ${C.hair}`, paddingTop: 9 }}>{rd.note}</div>
            </>, { borderLeft: `2.5px solid ${BC}` })}</div>); })()}
          {fuel && <div style={{ marginBottom: 10 }}>{card(<>
            {sectionTitle("Fuel guard", C.caution)}
            <div style={{ fontSize: 13.5, color: C.caution, fontWeight: 700, lineHeight: 1.5 }}>{fuel}</div>
          ...
          {nextSlot && (() => { // v0.9.69: mock's warm-up / cool-down movement chips, chosen by session focus
            ...
            const row = (title, list) => (<div style={{ marginBottom: 10 }}>{card(<>
              ...
              <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
                {list.map((m) => (<span key={m} style={{ fontFamily: DATA, fontSize: 11.5, fontWeight: 700, letterSpacing: 0.8, color: C.ink2, border: `1px solid ${C.hair}`, background: "rgba(255,255,255,0.03)", borderRadius: 999, padding: "7px 13px" }}>{m}</span>))}
              </div>
            </>)}</div>);
          ...
          {nextSlot ? card(<div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 3, gap: 8 }}>
              <div style={{ fontFamily: DISPLAY, fontSize: 21, fontWeight: 700, color: C.ink }}>{nextSlot.day.name}</div>
              {(() => { const mins = Math.round((nextSlot.day.exercises || []).reduce((n, e) => n + ((e.sets || 3) * 2.6 + 1.5), 0));
                return mins > 0 ? <span style={{ fontFamily: DATA, fontSize: 10, fontWeight: 700, letterSpacing: 1, color: C.go, border: `1px solid ${C.go}55`, background: C.go + "1A", borderRadius: 999, padding: "4px 10px", whiteSpace: "nowrap", flexShrink: 0 }}>START · ~{mins} MIN</span> : null; })()}
            </div>
              ...
              return (<div key={i} style={{ padding: "8px 0", borderTop: i ? `1px solid ${C.hair}` : "none" }}>
                {startsCore && (
                  <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 8, marginBottom: 5 }}>
                    <div style={{ fontSize: 11.5, letterSpacing: 1.4, color: C.muted, fontWeight: 800 }}>CORE</div>
                    <a href={sixpackUrl()} rel="noopener" onClick={() => { const t = setTimeout(() => { if (!document.hidden) window.location.href = SIXPACK.store; }, sixpackFallbackDelay(document.hidden)); const stop = () => clearTimeout(t); document.addEventListener("visibilitychange", stop, { once: true }); window.addEventListener("pagehide", stop, { once: true }); }} style={{ fontSize: 11.5, color: C.violet, fontWeight: 700, textDecoration: "none", whiteSpace: "nowrap" }}>or use {SIXPACK.label} →</a>
                  </div>
                )}
                <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                  <div style={{ fontSize: 14.5, color: C.ink, fontWeight: 700 }}>{x.name}</div>
                ...
                <div style={{ fontSize: 12.5, color: adv.action === "add-weight" ? C.go : adv.action === "deload" ? C.caution : C.faint, marginTop: 2 }}>
            ...
            <button onClick={() => startSession(nextSlot.day)} style={{ marginTop: 12, width: "100%", background: C.go, color: C.bg, border: "none", borderRadius: 12, padding: "13px 0", fontFamily: DATA, fontSize: 13.5, fontWeight: 800, letterSpacing: 1.4, textTransform: "uppercase", cursor: "pointer", boxShadow: `0 4px 16px ${C.go}33` }}>Start session</button>
          </div>, { marginBottom: 12 }) : card(<div style={{ fontSize: 14, color: C.muted }}>Rest day — next session is on the Week view.</div>, { marginBottom: 12 })}
          {card(<div>
            {sectionTitle("Log cardio")}
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 9 }}>{CARDIO_TYPES.map((t) => (
              <button key={t} onClick={() => setCardioDraft((d) => ({ ...d, type: t }))} style={{ background: cardioDraft.type === t ? C.go + "14" : "transparent", color: cardioDraft.type === t ? C.go : C.muted, border: `1px solid ${cardioDraft.type === t ? C.go + "55" : C.hair}`, borderRadius: 999, padding: "7px 12px", fontFamily: DATA, fontSize: 10.5, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", cursor: "pointer" }}>{t}</button>))}</div>
            <div style={{ display: "flex", gap: 7, alignItems: "center" }}>
              ...
              <span style={{ fontSize: 13.5, color: C.muted }}>min</span>
              {["easy", "zone 2", "hard"].map((iv) => (
                <button key={iv} onClick={() => setCardioDraft((d) => ({ ...d, intensity: iv }))} style={{ flex: 1, background: cardioDraft.intensity === iv ? C.go : C.surfaceAlt, color: cardioDraft.intensity === iv ? C.surface : C.muted, border: "none", borderRadius: 9, padding: "10px 4px", fontFamily: BODY, fontSize: 12.5, fontWeight: 700, cursor: "pointer" }}>{iv}</button>))}
            </div>
            <button onClick={() => { const mins = +cardioDraft.minutes || 0; if (!mins) return;
              ...
              style={{ marginTop: 10, width: "100%", background: C.surfaceAlt, border: `1.5px solid ${C.hair}`, color: C.ink, borderRadius: 10, padding: "10px 0", fontFamily: BODY, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>Log {cardioDraft.minutes || 0} min {cardioDraft.type}</button>
            <div style={{ fontSize: 11.5, color: C.faint, marginTop: 7 }}>{cardioMin7} of {cardioTarget(routine.goal)} min this week · cardio helps the deficit but competes for recovery — it is placed around lifting, not piled on top.</div>
          </div>, { marginBottom: 12 })}
          {card(<div>{sectionTitle("This week")}<div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            {[["sessions", `${done7}×`], ["hard sets", String(Object.values(sets7).reduce((n, v) => n + v, 0))], ["goal", routine.goal]].map(([l, v]) => (
              <div key={l} style={{ flex: "1 1 30%", background: C.surfaceAlt, borderRadius: 10, padding: "9px 10px" }}>
                <div style={{ fontSize: 16, fontWeight: 800, color: C.ink }}>{v}</div><div style={{ fontSize: 11.5, color: C.muted }}>{l}</div>
              </div>))}
          </div></div>)}
        </div>))}
        {trainView === "week" && card(<div>
          ...
          {(() => { const W = (week.length ? week : trainDates()); if (!W.length) return null;
            ...
            return (<div style={{ display: "flex", alignItems: "flex-end", gap: 6, height: 46, marginBottom: 12 }}>
              {W.map((d, i) => (<div key={i} style={{ flex: 1, textAlign: "center" }}>
                ...
                <div style={{ fontFamily: DATA, fontSize: 9.5, color: (d.dose || d.after) ? C.violet : C.faint, fontWeight: 700 }}>{d.label || ""}</div>
              </div>))}
            </div>); })()}
          {(week.length ? week : trainDates()).map((d, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, padding: "9px 0", borderTop: i ? `1px solid ${C.hair}` : "none" }}>
              <div><div style={{ fontSize: 14.5, fontWeight: 700, color: d.day ? C.ink : C.faint }}>{d.label}{(d.dose || d.after) ? " · shot window" : ""}</div>
                {d.note && <div style={{ fontSize: 12, color: C.violet, marginTop: 2, lineHeight: 1.4 }}>{d.note}</div>}</div>
              <div style={{ fontSize: 13.5, color: d.day ? C.go : C.faint, fontWeight: 700, textAlign: "right" }}>{d.day ? d.day.name : (() => { const c = (weekCardio.find((x) => x.iso === d.iso) || {}).cardio; return c ? `${c.minutes} min ${c.intensity}` : "rest"; })()}</div>
            </div>))}
        </div>)}
        {trainView === "lifts" && (workoutLog.length === 0 ? card(<div style={{ fontSize: 14, color: C.muted, lineHeight: 1.5 }}>No sessions logged yet. Finish one and this fills with per-lift bests, estimated one-rep maxes, and weekly volume by muscle.</div>) : (<div>
          {nextSlot && (() => { const foci = (nextSlot.day.focus || []).join(" ").toLowerCase();
            ...
            return (<div style={{ margin: "10px 0" }}>{card(<>
              ...
              <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
                {down.map((m) => (<span key={m} style={{ fontFamily: DATA, fontSize: 11.5, fontWeight: 700, letterSpacing: 0.8, color: C.ink2, border: `1px solid ${C.hair}`, background: "rgba(255,255,255,0.03)", borderRadius: 999, padding: "7px 13px" }}>{m}</span>))}
              </div>
            </>)}</div>); })()}
          {card(<div><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>{sectionTitle("Weekly sets by muscle")}<span style={{ fontFamily: DATA, fontSize: 9.5, fontWeight: 700, letterSpacing: 1, borderRadius: 999, padding: "3px 9px", marginTop: -8, color: C.go, border: `1px solid ${C.go}55`, background: C.go + "1A" }}>TARGET 10–20</span></div>
            {Object.entries(sets7).sort((a, b) => b[1] - a[1]).map(([g, n], i) => (
              <div key={g} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderTop: i ? `1px solid ${C.hair}` : "none" }}>
                <span style={{ fontFamily: DATA, fontSize: 11.5, fontWeight: 700, letterSpacing: 1, color: C.ink2, textTransform: "uppercase" }}>{({ shoulders: "SHLDR", hamstrings: "HAMS", quads: "QUADS", triceps: "TRI", biceps: "ARMS", glutes: "GLUTES", calves: "CALVES", chest: "CHEST", back: "BACK", core: "CORE" }[g] || g)}</span>
                <span style={{ fontFamily: DATA, fontSize: 13.5, color: n >= 10 ? C.go : C.caution, fontWeight: 700 }}>{n} sets{n < 10 ? " · light" : ""}</span>
              </div>))}
            <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderTop: `1px solid ${C.hair}` }}>
              <span style={{ fontSize: 14, color: C.ink }}>cardio</span>
              <span style={{ fontSize: 14, color: cardioMin7 >= cardioTarget((routine || {}).goal) ? C.go : C.muted, fontWeight: 700 }}>{cardioMin7} / {cardioTarget((routine || {}).goal)} min</span>
            </div>
            <div style={{ fontSize: 11.5, color: C.faint, marginTop: 7 }}>10–20 hard sets per muscle per week is the usual growth range; less still protects muscle while cutting.</div>
          </div>, { marginBottom: 12 })}
          {card(<div>{sectionTitle("Lifts · best estimated 1RM")}
            {[...new Set(workoutLog.flatMap((s) => (s.entries || []).map((e) => e.exId)))].map((id, i) => {
              ...
              return (<div key={id} style={{ display: "flex", justifyContent: "space-between", gap: 8, padding: "7px 0", borderTop: i ? `1px solid ${C.hair}` : "none" }}>
                <span style={{ fontSize: 14, color: C.ink }}>{name}</span>
                <span style={{ fontSize: 13.5, color: C.muted, whiteSpace: "nowrap" }}>{best} lb {delta > 0 ? <b style={{ color: C.go }}>+{delta}</b> : delta < 0 ? <b style={{ color: C.caution }}>{delta}</b> : ""} · {h.length}×</span>
              </div>); })}
          </div>)}
        </div>))}
        {trainView === "setup" && card(<div>
          ...
          <div style={{ display: "flex", gap: 7, flexWrap: "wrap", marginBottom: 12 }}>{["bodyweight", "dumbbell", "barbell", "cable", "machine", "kettlebell", "bands", "ball", "other"].map((e) => {
            ...
            return (<button key={e} onClick={() => setTrainPrefs((t) => ({ ...t, equipment: on ? t.equipment.filter((x) => x !== e) : [...t.equipment, e] }))} style={{ background: on ? C.go + "14" : "transparent", color: on ? C.go : C.muted, border: `1px solid ${on ? C.go + "55" : C.hair}`, borderRadius: 999, padding: "7px 12px", fontFamily: DATA, fontSize: 10.5, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", cursor: "pointer" }}>{({ bodyweight: "Bodyweight", dumbbell: "Dumbbells", barbell: "Barbell", cable: "Cables", machine: "Machines", kettlebell: "Kettlebell", bands: "Bands", ball: "Ball", other: "Other" })[e] || e}</button>); })}</div>
          ...
          <input value={trainPrefs.videoChannel || ""} onChange={(e) => setTrainPrefs((t) => ({ ...t, videoChannel: e.target.value.replace(/^@/, "").replace(/[^A-Za-z0-9_.-]/g, "") }))} placeholder="leave blank — searches all of YouTube" autoCapitalize="none" autoCorrect="off" spellCheck={false}
            style={{ width: "100%", boxSizing: "border-box", background: C.surfaceAlt, border: `1.5px solid ${C.hair}`, borderRadius: 10, padding: "11px 12px", color: C.ink, fontFamily: BODY, fontSize: 14, marginBottom: 4 }} />
          <div style={{ fontSize: 11.5, color: C.faint, marginBottom: 12, lineHeight: 1.45 }}>Handle only, no @. Demos are searched inside this channel first — leave blank to search all of YouTube for short clips instead.</div>
          <button onClick={() => { const r = buildRoutine(exCatalog, trainPrefs); setRoutine(r); setTrainView("today"); }} disabled={!exCatalog.length || !trainPrefs.equipment.length}
            style={{ width: "100%", background: exCatalog.length && trainPrefs.equipment.length ? C.go : C.hair, color: C.surface, border: "none", borderRadius: 12, padding: "13px 0", fontFamily: BODY, fontSize: 15.5, fontWeight: 800, cursor: "pointer" }}>{routine ? "Rebuild routine" : "Generate routine"}</button>
          {routine && <div style={{ fontSize: 12.5, color: C.faint, marginTop: 9, lineHeight: 1.5 }}>{routine.blurb} Heavy days are kept off your shot day and the day after — your shot day eased around the injection; work nights eased too. Sessions log to Apple Health as strength training, which feeds the lean-mass engine on the Body tab.</div>}
        </div>)}
      </div>
    );
  };
  const renderBody = () => (
    <div style={{ padding: "18px 18px 12px" }}>
      <div style={{ fontFamily: DISPLAY, fontSize: 25, fontWeight: 700, color: C.ink }}>Body</div>
      <div style={{ fontSize: 14, color: C.muted, marginBottom: 16 }}>Composition, trend &amp; progress</div>
      ...
      {(() => { // v0.9.63: Path to your forecast — the EXISTING goalContract engine wearing the mock's
        ...
        return (<div style={{ marginBottom: 14 }}>{card(<>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            ...
            <span style={{ fontFamily: DATA, fontSize: 9.5, fontWeight: 700, letterSpacing: 1, color: allOk ? C.go : C.caution, border: `1px solid ${(allOk ? C.go : C.caution)}55`, background: (allOk ? C.go : C.caution) + "1A", borderRadius: 999, padding: "3px 9px", marginTop: -8 }}>{allOk ? "ON TRACK" : "CHECK SIGNALS"}</span>
          </div>
          <div style={{ display: "flex", alignItems: "flex-end", gap: 16 }}>
            <div><span style={{ fontFamily: DATA, fontSize: 39, fontWeight: 700, color: C.ink }}><span style={{ display: "inline-block", width: 6, height: 6, borderRadius: 6, background: C.go, marginRight: 8, verticalAlign: "middle" }} />{fmtWt(cur)}</span><span style={{ fontFamily: DATA, fontSize: 14, color: C.faint }}> {wtU}</span></div>
            <div style={{ paddingBottom: 5 }}>
              <span style={{ fontFamily: DATA, fontSize: 10, letterSpacing: 1.2, color: C.faint, textTransform: "uppercase" }}>Goal</span>
            ...
            <div style={{ marginLeft: "auto", paddingBottom: 5, textAlign: "right" }}>
              <div style={{ fontFamily: DATA, fontSize: 14, fontWeight: 700, color: C.go }}>{lost > 0 ? `−${lost.toFixed(1)}` : "±0"} {wtU}</div>
              <div style={{ fontFamily: DATA, fontSize: 10, letterSpacing: 1, color: C.faint, textTransform: "uppercase" }}>since start</div>
            </div>
          </div>
          {weightSeries.length > 1 ? (() => { // v0.9.66: the mock's own weight chart — gradient fill, glow endpoint, dashed goal rule
            ...
            </svg>); })() : <div style={{ padding: "26px 0", textAlign: "center", color: C.faint, fontSize: 14 }}>Log your first weight below to start the trend.</div>}
          {ct.status !== "incomplete" && <div style={{ display: "flex", gap: 7, marginTop: 10, marginBottom: 8 }}>
            {[[Math.round(ct.lean || 0) + " lb", "LEAN TO HOLD"], [Math.round(ct.fatToLose || 0) + " lb", "FAT TO LOSE"], [ct.bfAtGoal ? "~" + Math.round(ct.bfAtGoal) + "%" : "—", "AT GOAL"]].map(([v4, l4]) => (
              <div key={l4} style={{ flex: 1 }}>
                <div style={{ fontFamily: DATA, fontSize: 10, letterSpacing: 1.1, color: C.faint, textTransform: "uppercase" }}>{l4}</div>
                <div style={{ fontFamily: DATA, fontSize: 16, fontWeight: 700, color: C.ink, marginTop: 3 }}>{v4}</div>
              </div>))}
          </div>}
          {ct.status !== "incomplete" && (ct.weeksTarget || ct.weeksAtCurrent) && <div style={{ fontFamily: DATA, fontSize: 11.5, color: C.muted, marginBottom: 8 }}>{ct.weeksTarget ? `${ct.weeksTarget} wk at target rate` : ""}{ct.weeksAtCurrent ? `${ct.weeksTarget ? " · " : ""}${ct.weeksAtCurrent} wk at measured rate` : ""}</div>}
          {ct.status === "incomplete" && <div style={{ fontSize: 12.5, color: C.faint, marginBottom: 8 }}>{ct.reason}</div>}
          <div style={{ borderTop: `1px solid ${C.hair}` }}>
            {rows.map((r4) => (<div key={r4.l} style={{ display: "flex", alignItems: "center", gap: 9, padding: "8px 0", borderBottom: `1px solid ${C.hair}` }}>
              ...
              <span style={{ fontSize: 13.5, color: C.muted, flex: 1 }}>{r4.l}</span>
              <span style={{ fontFamily: DATA, fontSize: 10, fontWeight: 700, letterSpacing: 0.7, color: PC[r4.st], border: `1px solid ${PC[r4.st]}55`, background: PC[r4.st] + "14", borderRadius: 999, padding: "3px 9px" }}>{r4.st === "ok" ? "OK" : r4.st === "flag" ? "MISS" : "WAIT"} · {r4.v}</span>
            </div>))}
          </div>
          {pAvg != null && pAvg < pFloor && <button onClick={() => setTargets((t) => ({ ...t, protein: pFloor }))} style={{ width: "100%", marginTop: 11, background: "transparent", border: `1px solid ${C.hair}`, color: C.ink, borderRadius: 12, padding: "13px 0", fontFamily: BODY, fontSize: 14.5, fontWeight: 600, cursor: "pointer" }}>Raise protein target to {pFloor} g</button>}
          {weightSeries.length > 0 && <div style={{ marginTop: 10 }}>{[...weightSeries].slice(-3).reverse().map((w, i) => (
            <div key={w.date + i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0", borderBottom: `1px solid ${C.hair}` }}>
              ...
              {w.synced ? <span style={{ fontFamily: DATA, fontSize: 9.5, fontWeight: 700, color: C.muted, letterSpacing: 0.8, border: `1px solid ${C.hair}`, borderRadius: 999, padding: "3px 8px" }}>{w.source ? ({ "apple-health": "APPLE HEALTH", "google-health": "GOOGLE HEALTH" }[w.source] || w.source.toUpperCase().replace(/-/g, " ")) : "SYNCED"}</span> : <button onClick={() => setWeightLog((l) => l.filter((x) => !(x.date === w.date && x.lbs === w.lbs)))} style={{ background: "none", border: "none", color: C.faint, fontSize: 15, cursor: "pointer", padding: 4 }}>✕</button>}
            </div>
          ))}</div>}
          <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
            <input type="number" value={newWeight} placeholder={`Log today's weight (${wtU})`} onChange={(e) => setNewWeight(e.target.value)} step="0.1" style={{ flex: 1, fontFamily: DATA, fontSize: 16, fontWeight: 600, color: C.ink, background: C.surfaceAlt, border: `1px solid ${C.hair}`, borderRadius: 10, padding: "11px 13px", outline: "none", boxSizing: "border-box" }} />
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 14 }}>
        {card(<>{stat("BMI", bmi ? bmi.toFixed(1) : "—", "")}<div style={{ fontSize: 12, color: C.faint, marginTop: 2 }}>{bmiBand(bmi)}</div></>, { flex: 1 })}
        {card(<>{stat("Body fat", bfShown ? bfShown.toFixed(1) : "—", "%")}<div style={{ fontSize: 12, color: C.faint, marginTop: 2 }}>{bfSource}</div></>, { flex: 1 })}
        {card(<>{stat("Lean", leanShown ? leanShown.toFixed(0) : "—", " lb")}<div style={{ fontSize: 12, color: C.faint, marginTop: 2 }}>{leanShown ? (bfMeasured ? "measured" : "fat-free mass") : "needs body fat"}</div></>, { flex: 1 })}
      </div>
      ...
      <div style={{ marginBottom: 14 }}>{card(<>
        {(() => { const ds0 = ((healthSync && healthSync.days) || []).filter((d0) => d0.bodyFatPct != null || d0.muscleMassLbs != null || d0.visceralFat != null);
          ...
          return (<div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            {nM > 0 && <span style={{ fontFamily: DATA, fontSize: 9.5, fontWeight: 700, letterSpacing: 1, borderRadius: 999, padding: "3px 9px", marginTop: -8, color: C.go, border: `1px solid ${C.go}55`, background: C.go + "1A" }}>{nM} METRICS</span>}
          </div>); })()}
        <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.45, marginBottom: 9 }}>A body-composition report carries more than Apple Health can store — visceral fat, body water and per-limb figures have no HealthKit fields. Snap or upload the report and the numbers come across.</div>
          ...
          return (<div style={{ marginBottom: 10 }}>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
              {tiles.map(([l5, v5]) => (<div key={l5} style={{ flex: "1 1 30%", background: "rgba(255,255,255,0.03)", border: `1px solid ${C.hair}`, borderRadius: 10, padding: "9px 10px" }}>
                ...
                <div style={{ fontFamily: DATA, fontSize: 9.5, letterSpacing: 0.9, color: C.faint, marginTop: 2 }}>{l5}</div>
              </div>))}
            </div>
            <div style={{ fontFamily: DATA, fontSize: 10, letterSpacing: 0.8, color: C.go, marginTop: 7 }}>MEASURED · {sc.date || "LAST SCAN"} — outranks the Navy estimate.</div>
          </div>); })()}
        <button onClick={() => scanRef.current && scanRef.current.click()} disabled={scanBusy}
          style={{ width: "100%", background: C.surfaceAlt, border: `1.5px dashed ${C.hair}`, color: C.ink, borderRadius: 11, padding: "12px 0", fontFamily: BODY, fontSize: 14, fontWeight: 700, cursor: "pointer", opacity: scanBusy ? 0.6 : 1 }}>
            ...
            <div style={{ marginTop: 11, background: C.surfaceAlt, border: `1px solid ${C.hair}`, borderRadius: 11, padding: "11px 13px" }}>
              ...
              {ROWS.map(([k, val, u]) => (
                <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "3px 0", fontSize: 14 }}>
                  <span style={{ color: C.muted }}>{k}</span><span style={{ fontFamily: DATA, color: C.ink, fontWeight: 600, fontSize: 13.5 }}>{val}{u}</span>
                </div>
              ))}
              {v.segmental && <div style={{ marginTop: 6, fontSize: 12, color: C.faint, lineHeight: 1.4 }}>Per-limb figures captured for reference. Scales model these rather than measuring each limb, so they are shown but never used to judge left/right balance.</div>}
              {bodyScan.warning && <div style={{ marginTop: 8, fontSize: 13, color: C.avoid, lineHeight: 1.45 }}>⚠ {bodyScan.warning}</div>}
              {!!bodyScan.rejected.length && <div style={{ marginTop: 6, fontSize: 12, color: C.faint }}>Skipped as unreadable or out of range: {bodyScan.rejected.join(", ")}</div>}
              <div style={{ display: "flex", gap: 8, marginTop: 11 }}>
                ...
                <button onClick={() => setBodyScan(null)} style={{ flex: 1, background: "none", color: C.muted, border: `1px solid ${C.hair}`, borderRadius: 10, padding: "11px 0", fontFamily: BODY, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>Discard</button>
              </div>
            </div>
          );
        })()}
      ...
      <div style={{ marginBottom: 14 }}>{card(
        <>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <span style={{ fontFamily: DATA, fontSize: 9.5, fontWeight: 700, letterSpacing: 1, color: C.muted, border: `1px solid ${C.hair}`, borderRadius: 999, padding: "3px 9px", marginTop: -8 }}>NAVY ESTIMATE</span>
          </div>
          <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>{["male", "female"].map((s) => (<button key={s} onClick={() => setBody({ ...body, sex: s })} style={{ flex: 1, padding: "9px 0", borderRadius: 999, border: `1.5px solid ${body.sex === s ? C.go : C.hair}`, background: body.sex === s ? C.go + "22" : "transparent", color: body.sex === s ? C.go : C.muted, fontFamily: DATA, fontSize: 12, fontWeight: 700, letterSpacing: 1, cursor: "pointer", textTransform: "uppercase" }}>{s}</button>))}</div>
      ...
      <div style={{ marginBottom: 14 }}>{card(<>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <span style={{ fontFamily: DATA, fontSize: 9.5, fontWeight: 700, letterSpacing: 1, borderRadius: 999, padding: "3px 9px", marginTop: -8, color: C.muted, border: `1px solid ${C.hair}` }}>8 SECTIONS</span>
        </div>
        <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.45, marginBottom: 10 }}>Everything this app has measured about you, printed for a clinic visit: your per-dose response, your protocol and checkpoint, tolerability surveillance, training and intake. It reports; it never recommends a dose.</div>
        <button onClick={exportPDF} style={{ width: "100%", background: C.go, color: C.bg, border: "none", borderRadius: 999, padding: "12px 0", fontFamily: DATA, fontSize: 13, fontWeight: 800, letterSpacing: 1, textTransform: "uppercase", cursor: "pointer", boxShadow: `0 4px 14px ${C.go}44` }}>Generate PDF</button>
      </>)}</div>
      ...
      {card(
        <>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <span style={{ fontFamily: DATA, fontSize: 9.5, fontWeight: 700, letterSpacing: 1, borderRadius: 999, padding: "3px 9px", marginTop: -8, color: C.caution, border: `1px solid ${C.caution}55`, background: C.caution + "1A" }}>FRONT / BACK</span>
          </div>
          {photos.length === 0 && (
            <div style={{ textAlign: "center", padding: "24px 12px", color: C.faint, fontSize: 14, lineHeight: 1.5 }}>Add photos to build a visual transformation timeline.<br /><span style={{ fontSize: 12 }}>Stored privately on your node.</span></div>
          )}
  ...
  const rhrCardFor = (_rr) => { const RED = "#f05252";
    ...
    return <div style={{ marginBottom: 14 }}>{card(<div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            {_rr.status === "collecting" ? <span style={{ fontFamily: DATA, fontSize: 9.5, fontWeight: 700, letterSpacing: 1, borderRadius: 999, padding: "3px 9px", marginTop: -8, color: C.muted, border: `1px solid ${C.hair}` }}>COLLECTING {_rr.have}/{_rr.need}</span>
             : _rr.flagged ? <span style={{ fontFamily: DATA, fontSize: 9.5, fontWeight: 700, letterSpacing: 1, borderRadius: 999, padding: "3px 9px", marginTop: -8, color: RED, border: `1px solid ${RED}66`, background: "rgba(240,82,82,0.12)" }}>FLAGGED</span> : null}
          </div>
          {_rr.status === "empty" && <div style={{ fontSize: 13.5, color: C.muted, marginTop: 8, lineHeight: 1.5 }}>
          ...
          {_rr.status === "collecting" && <div style={{ fontSize: 13.5, color: C.muted, marginTop: 8 }}>
          ...
          {_rr.status === "ready" && <div>
            <div style={{ display: "flex", alignItems: "baseline", gap: 10, marginTop: 8 }}>
              <span style={{ fontFamily: DATA, fontSize: 31, fontWeight: 700, color: C.ink }}>{_rr.current}</span>
              <span style={{ fontSize: 13, color: C.faint }}>bpm today</span>
              <span style={{ marginLeft: "auto", fontSize: 13.5, fontWeight: 700, color: _rr.delta >= 8 ? RED : C.good }}>{_rr.delta >= 0 ? "+" : ""}{_rr.delta} vs your baseline ({_rr.baseline})</span>
            </div>
            ...
            {_rr.flagged
              ? <div style={{ background: "rgba(240,82,82,0.10)", border: "1px solid rgba(240,82,82,0.35)", borderRadius: 10, padding: "9px 11px", marginTop: 9, fontSize: 13.5, color: C.ink, lineHeight: 1.5 }}>
              ...
              : <div style={{ fontSize: 12, color: C.faint, marginTop: 8 }}>Tracking within your baseline. Flags only a rise of 8+ bpm sustained 7+ days — single spiky days are ignored. Works for any medication on your list.</div>}
          </div>}
  ...
  const renderGlp = () => (
    <div style={{ padding: "18px 18px 12px" }}>
      <div style={{ fontFamily: DISPLAY, fontSize: 25, fontWeight: 700, color: C.ink }}>GLP-1</div>
      <div style={{ fontSize: 14, color: C.muted, marginBottom: 16 }}>Medication · titration · tolerability</div>
            ...
            {PHASES.map((p, i) => {
              ...
              return (
                <div key={p.key} style={{ flex: 1 }}>
                  ...
                  <div style={{ fontFamily: DATA, fontSize: 10, letterSpacing: 0.4, marginTop: 5, fontWeight: cur ? 700 : 500, color: cur ? C.violet : C.faint, textAlign: "center", lineHeight: 1.2, textTransform: "uppercase" }}>{p.label}</div>
                </div>
              );
            })}
          </div>
          <div style={{ background: C.violet + "12", borderRadius: 12, padding: "12px 14px" }}>
            <div style={{ fontSize: 14.5, fontWeight: 700, color: C.violet }}>{PHASES[phaseIdx].label}</div>
            <div style={{ fontSize: 13.5, color: C.ink2, marginTop: 3, lineHeight: 1.45 }}>{PHASES[phaseIdx].focus}</div>
          </div>
          <div style={{ fontSize: 11.5, color: C.faint, marginTop: 10, lineHeight: 1.4 }}>Most apps quit at "goal reached." The regain problem lives in maintenance &amp; coming off the drug — this is built to carry you through it.</div>
        </>)}</div>
          ...
          <div style={{ marginBottom: 14 }}>{card(<div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                {curMg != null && rungs.length > 0 && rungs.findIndex((rr9) => +rr9 === +curMg) >= 0 && <span style={{ fontFamily: DATA, fontSize: 9.5, fontWeight: 700, letterSpacing: 1, borderRadius: 999, padding: "3px 9px", marginTop: -8, color: C.violet, border: `1px solid ${C.violet}55`, background: C.violet + "1A" }}>RUNG {rungs.findIndex((rr9) => +rr9 === +curMg) + 1} OF {rungs.length}</span>}
              </div>
              <span onClick={() => { if (!protoEdit) setProtoRungs(rungs.join(", ")); setProtoEdit((v) => !v); }} style={{ fontFamily: DATA, fontSize: 11, fontWeight: 700, letterSpacing: 1.2, textTransform: "uppercase", color: C.violet, cursor: "pointer", marginTop: -8 }}>{protoEdit ? "done" : "edit"}</span>
            </div>
            ...
            {protoEdit ? (<div style={{ marginTop: 11 }}>
              ...
              {(() => { const v = parseRungs(protoRungs); return v.length ? (
                <div style={{ fontSize: 12, color: C.go, marginTop: 6, fontVariantNumeric: "tabular-nums" }}>{"Will save: " + v.join(" \u2192 ") + " mg"}</div>
              ) : protoRungs.trim() ? (
                <div style={{ fontSize: 12, color: C.caution, marginTop: 6 }}>No readable numbers yet</div>
              ) : null; })()}
              <div style={{ fontSize: 12, color: C.muted, margin: "10px 0 5px" }}>Minimum hold before a checkpoint</div>
              <div style={{ display: "flex", gap: 6 }}>
                {[4, 5, 6, 8].map((wk) => (<button key={wk} onClick={() => setGlp((g) => ({ ...g, protocol: { ...(g.protocol || {}), minHoldDays: wk * 7 } }))}
                  style={{ flex: 1, background: holdWk === wk ? C.hair : "transparent", border: `1px solid ${C.hair}`, color: holdWk === wk ? C.ink : C.muted, borderRadius: 9, padding: "9px 0", fontSize: 13.5, fontWeight: holdWk === wk ? 700 : 500, fontFamily: BODY, cursor: "pointer" }}>{wk} wk</button>))}
              </div>
              <button onClick={() => { const v = parseRungs(protoRungs);
                  setGlp((g) => ({ ...g, protocol: { ...(g.protocol || {}), rungs: v.length ? v : null } })); setProtoEdit(false); }}
                style={{ width: "100%", marginTop: 10, background: C.go, color: C.surface, border: "none", borderRadius: 10, padding: "11px 0", fontFamily: BODY, fontSize: 14.5, fontWeight: 700, cursor: "pointer" }}>Save protocol</button>
            </div>) : (
              <div style={{ marginTop: 11 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 0", borderTop: `1px solid ${C.hair}` }}>
                  <span style={{ fontSize: 14, color: C.ink2 }}>Minimum hold</span>
                  <span style={{ fontFamily: DATA, fontSize: 13.5, fontWeight: 700, color: C.ink }}>{holdWk} wk</span>
                </div>
                {rungs.length > 0 && <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 0", borderTop: `1px solid ${C.hair}` }}>
                  <span style={{ fontSize: 14, color: C.ink2 }}>Your ceiling</span>
                  <span style={{ fontFamily: DATA, fontSize: 13.5, fontWeight: 700, color: C.ink }}>{rungs[rungs.length - 1]} mg</span>
                </div>}
                <div style={{ fontSize: 11.5, color: C.faint, marginTop: 4 }}>Minimum hold {holdWk} weeks · you and your prescriber set these</div>
              </div>)}
          ...
          <div style={{ marginBottom: 14 }}>{card(<div>
            ...
            {noDose ? (<div>
              <div style={{ border: `1px dashed ${C.hair}`, borderRadius: 11, padding: "13px 0", textAlign: "center",
                color: C.faint, fontFamily: BODY, fontSize: 14.5, fontWeight: 700 }}>
            ...
            {cp.veto.length > 0 && (
              <div style={{ background: "rgba(240,82,82,0.10)", border: `1px solid rgba(240,82,82,0.35)`,
                borderRadius: 10, padding: "9px 11px", marginBottom: 11, fontSize: 13.5, color: C.ink, lineHeight: 1.5 }}>
            ...
            {cp.veto.length === 0 && cp.status !== "nodose" && rr.status === "ready" && sl.status === "ready" && (
              <div style={{ fontSize: 12, color: C.faint, marginBottom: 11 }}>Tolerability flags right now: none — RHR steady, sleep steady.</div>)}

            {locked ? (<div>
              <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginBottom: 7 }}>
                <span style={{ width: 7, height: 7, borderRadius: 7, background: C.go, alignSelf: "center", flexShrink: 0 }} />
                <span style={{ fontFamily: DATA, fontSize: 31, fontWeight: 700, color: C.ink, lineHeight: 1 }}>{cp.days}</span>
                <span style={{ fontFamily: DATA, fontSize: 14, color: C.muted }}>/ {cp.need} days at {cp.cur} mg</span>
              </div>
              ...
              <button disabled style={{ width: "100%", background: "transparent", border: `1px dashed ${C.hair}`,
                color: C.faint, borderRadius: 11, padding: "13px 0", fontFamily: BODY, fontSize: 14.5, fontWeight: 700 }}>
              ...
              <button onClick={() => setCpOpenFor(cpOpen ? null : cp.cur)}
                style={{ width: "100%", background: cpOpen ? "transparent" : C.violet, color: cpOpen ? C.ink : C.surface,
                  ...
                  fontFamily: BODY, fontSize: 14.5, fontWeight: 700, cursor: "pointer" }}>
              ...
              {!cpOpen && cp.stall.on && <div style={{ background: C.cautionSoft || "rgba(240,178,82,0.10)",
                ...
                marginTop: 11, fontSize: 13.5, color: C.ink, lineHeight: 1.5 }}>
              ...
              {!cpOpen && <div style={{ fontSize: 12, color: C.faint, marginTop: 9, lineHeight: 1.5 }}>
            ...
            {cpOpen && !locked && (<div style={{ marginTop: 14 }}>
              {cp.status === "ask" ? (<div>
                ...
                <div style={{ fontSize: 14, color: C.muted, lineHeight: 1.55 }}>Your measured markers are in. How is appetite between meals?</div>
                  ...
                  {[["controlled", "Controlled"], ["returning", "Returning"], ["suppressed", "Fully gone"]].map(([k, lbl]) => (
                    <button key={k} onClick={() => setGlp((g) => ({ ...g, checkpointAnswers: { ...(g.checkpointAnswers || {}), [String(cp.cur)]: k } }))}
                      style={{ flex: 1, background: "transparent", border: `1px solid ${C.hair}`, color: C.ink, borderRadius: 10,
                        padding: "11px 0", fontFamily: BODY, fontSize: 13.5, fontWeight: 600, cursor: "pointer" }}>{lbl}</button>))}
                </div>
              </div>) : (<div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                  <span style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: 0.6, color: tone,
                  ...
                  <span onClick={() => setGlp((g) => ({ ...g, checkpointAnswers: { ...(g.checkpointAnswers || {}), [String(cp.cur)]: null } }))}
                    style={{ fontSize: 11.5, color: C.faint, cursor: "pointer" }}>re-answer</span>
                </div>
                <div style={{ fontSize: 17, fontWeight: 700, color: C.ink, margin: "2px 0 6px", lineHeight: 1.3 }}>{title}</div>
              ...
              <div style={{ marginTop: 13, borderTop: `1px solid ${C.hair}` }}>
                {cp.rows.map((r) => (
                  <div key={r.label} style={{ display: "flex", alignItems: "center", gap: 9, padding: "8px 0", borderBottom: `1px solid ${C.hair}` }}>
                    ...
                    <span style={{ fontSize: 13.5, color: C.muted, flex: 1 }}>{r.label}</span>
                    <span style={{ fontSize: 13, color: C.ink, textAlign: "right" }}>{r.value}</span>
                    <span style={{ fontFamily: DATA, fontSize: 10, fontWeight: 700, letterSpacing: 0.8,
              ...
              <div style={{ fontSize: 11.5, color: C.faint, marginTop: 11, lineHeight: 1.5 }}>
          ...
          <div style={{ marginBottom: 14 }}>{card(<div>
            {(() => { // v0.9.70: the fat-ceiling engine finally gets its card
              ...
              return (<div style={{ marginBottom: 12, paddingBottom: 12, borderBottom: `1px solid ${C.hair}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div style={{ fontFamily: DATA, fontSize: 11.5, fontWeight: 700, color: C.muted, letterSpacing: 1.6, textTransform: "uppercase", marginBottom: 10 }}>Your fat ceiling · dose window</div>
                  <span style={{ fontFamily: DATA, fontSize: 9.5, fontWeight: 700, letterSpacing: 1, borderRadius: 999, padding: "3px 9px", marginTop: -8, color: A9 ? C.go : C.faint, border: `1px solid ${A9 ? C.go + "55" : C.hair}`, background: A9 ? C.go + "1A" : "transparent" }}>{A9 ? "ACTIVE" : "COLLECTING"}</span>
                </div>
                {A9 ? (<div style={{ fontSize: 13.5, color: C.ink2, lineHeight: 1.55 }}>In your first 48 h post-dose, meals over <b style={{ fontFamily: DATA, color: C.caution }}>~{Math.round(fr.ceiling)} g fat</b> have run with more symptom days in your own log. Meal picks respect this ceiling automatically inside the window.</div>)
                : (<div style={{ fontSize: 13.5, color: C.muted, lineHeight: 1.55 }}>Learning your dose-window fat ceiling from your own logs — needs 5 symptom days and 10 meal days before a verdict.</div>)}
                <div style={{ display: "flex", gap: 10, marginTop: 9 }}>
                  {[[fr.inWin ?? 0, "In window"], [fr.days ?? 0, "Meal days"], [fr.sym ?? 0, "Sym days"]].map(([v7, l7]) => (
                    <div key={l7} style={{ flex: 1 }}>
                      <div style={{ fontFamily: DATA, fontSize: 10, letterSpacing: 1.1, color: C.faint, textTransform: "uppercase" }}>{l7}</div>
            ...
            {(() => {
              ...
              const cell = (lbl, v, kind, raw) => (
                <div style={{ flex: 1, minWidth: 58 }}>
                  <div style={{ fontFamily: DATA, fontSize: 10, letterSpacing: 1, color: C.muted, textTransform: "uppercase" }}>{lbl}</div>
                  <div style={{ fontSize: 13.5, fontWeight: 700, color: TONE_C[rungCellTone(kind, raw)], fontVariantNumeric: "tabular-nums" }}>{v == null ? "—" : v}</div>
                </div>);
              return (<div>
                {dr2.rungs.map((r) => (
                  <div key={r.mg} style={{ borderTop: `1px solid ${C.hair}`, padding: "10px 0 8px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 7 }}>
                      <span style={{ fontSize: 14, fontWeight: 700, color: C.ink }}>{r.mg} mg
                        <span style={{ fontSize: 11.5, fontWeight: 500, color: C.faint }}>{"\u2002·\u2002" + r.doses + (r.doses === 1 ? " dose" : " doses") + " · " + r.weeks + " wk" + (r.episodes > 1 ? " · " + r.episodes + " stays" : "")}</span></span>
                      <span style={{ fontFamily: DATA, fontSize: 10, fontWeight: 700, letterSpacing: 0.9, color: r.tier === "holding" ? C.go : C.faint,
                ...
                <div style={{ fontSize: 11.5, color: C.faint, marginTop: 9, lineHeight: 1.5 }}>
          ...
          {medObj.investigational ? (
            <div>
              <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 12 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 700, color: C.ink }}>{medObj.label}</div>
                  <div style={{ fontSize: 12.5, color: C.muted }}>{medObj.brand} · {medObj.cadence} · week {glp.weeksOn}</div>
              ...
              <div style={{ background: C.cautionSoft, border: `1px solid ${C.caution}55`, borderRadius: 12, padding: 11 }}>
                <div style={{ fontSize: 12.5, color: C.ink2, lineHeight: 1.45 }}><b style={{ color: C.caution }}>Investigational (Phase 3, not FDA-approved).</b> Tracking only — enter whatever your trial or clinician directs. No schedule is suggested, by design.</div>
              </div>
            </div>
          ) : (
            <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
              {donut(glp.dose, medObj.steps[medObj.steps.length - 1], C.violet, `${glp.dose}`, medObj.unit, C)}
              <div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 700, color: C.ink }}>{medObj.label}</div><div style={{ fontSize: 12.5, color: C.muted }}>{medObj.brand} · {medObj.cadence}</div><div style={{ fontFamily: DATA, fontSize: 12.5, color: C.muted, marginTop: 6 }}>Week {glp.weeksOn} · current dose {glp.dose} {medObj.unit}</div></div>
            </div>
          )}
        </>)}</div>
      ...
      <div style={{ marginBottom: 14 }}>{card(
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div><div style={{ fontFamily: DATA, fontSize: 11.5, fontWeight: 700, letterSpacing: 1.6, textTransform: "uppercase", color: C.muted }}>{medObj && medObj.cadence === "daily" ? "Next dose" : "Next injection"}</div><div style={{ fontFamily: DATA, fontSize: 27, fontWeight: 700, color: C.ink }}>{daysToInjection <= 0 ? "Today" : `${daysToInjection} day${daysToInjection > 1 ? "s" : ""}`}</div><div style={{ fontSize: 13, color: C.faint }}>{nextInjection.toLocaleDateString([], { weekday: "long", month: "short", day: "numeric" })}</div><div style={{ fontSize: 12, color: C.faint, marginTop: 3 }}>{glp.lastInjection ? `Last dose: ${fmtDate(glp.lastInjection)}${glp.dose ? ` · ${glp.dose} mg` : ""} · week ${glp.weeksOn}` : (medObj && medObj.cadence === "daily" ? "No dose logged yet — tap Log dose after your pill" : "No dose logged yet — tap Log dose after your injection")}</div></div>
          <button onClick={logInjection} style={{ background: doseLogged ? C.go : C.violet, color: C.bg, border: "none", borderRadius: 999, padding: "12px 20px", fontFamily: DATA, fontWeight: 800, fontSize: 13, letterSpacing: 1, textTransform: "uppercase", cursor: "pointer", boxShadow: `0 4px 16px ${(doseLogged ? C.go : C.violet)}44` }}>{doseLogged ? "Logged ✓" : "Log dose"}</button>
        </div>, { borderLeft: `2.5px solid ${C.violet}` })}</div>
      {(!medObj || medObj.cadence !== "daily") && <div style={{ marginBottom: 14 }}>{card(<>
        <div style={{ fontFamily: DATA, fontSize: 11.5, fontWeight: 700, letterSpacing: 1.6, textTransform: "uppercase", color: C.muted, marginBottom: 8 }}>Dose day{(prefs.injIntervalDays || 7) !== 7 ? <span style={{ textTransform: "none", color: C.faint }}> — applies to weekly schedules (yours is every {prefs.injIntervalDays} days)</span> : null}</div>
        <div style={{ display: "flex", gap: 6 }}>
          {[["SU", "Su"], ["MO", "Mo"], ["TU", "Tu"], ["WE", "We"], ["TH", "Th"], ["FR", "Fr"], ["SA", "Sa"]].map(([k, l]) => (
            <button key={k} onClick={() => setGlp({ ...glp, injectionDay: k })}
              style={{ flex: 1, height: 38, borderRadius: 999, border: `1.5px solid ${glp.injectionDay === k ? C.violet : C.hair}`, background: glp.injectionDay === k ? C.violet + "2E" : "transparent", color: glp.injectionDay === k ? C.violet : C.muted, fontFamily: DATA, fontWeight: 800, fontSize: 12, letterSpacing: 1, textTransform: "uppercase", cursor: "pointer", opacity: (prefs.injIntervalDays || 7) !== 7 ? 0.45 : 1 }}>{l}</button>
          ))}
        </div>
      </>)}</div>}
      {(!medObj || medObj.cadence !== "daily") && <div style={{ marginBottom: 14 }}>{card(<>
      ...
      {(() => {
        ...
        const weekChart = !weekReady ? null : (() => {
          ...
          return (<div>
            <div style={{ display: "flex", alignItems: "baseline", gap: 9, marginBottom: 4 }}>
              <span style={{ fontFamily: DATA, fontSize: 25, fontWeight: 700, color: C.ink }}>{hm(avg)}</span>
              <span style={{ fontFamily: DATA, fontSize: 12, color: C.faint }}>average</span>
              {sl.status === "ready" && <span style={{ marginLeft: "auto", fontFamily: DATA, fontSize: 12, fontWeight: 700, color: sl.flagged ? C.avoid : C.go }}>{sl.delta >= 0 ? "+" : ""}{sl.delta} min vs baseline</span>}
            </div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginTop: 10, paddingTop: 9, borderTop: `1px solid ${C.hair}` }}>
              {[["Deep", "#4C3FD4", 1], ["REM", "#67E8F9", 0.9], ["Light", "#3B84BC", 0.9], ["Awake", C.avoid, 0.55]].map(([l, col, op]) => (
                <span key={l} style={{ fontFamily: DATA, fontSize: 9.5, letterSpacing: 0.7, textTransform: "uppercase", color: C.muted }}>
          ...
          return (<div style={{ marginTop: 11 }}>
            {rows.map((r) => (
              <div key={r.k} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 7 }}>
                <div style={{ fontFamily: DATA, fontSize: 10, letterSpacing: 0.9, color: C.faint, textTransform: "uppercase", width: 34, flexShrink: 0 }}>{r.k}</div>
                ...
                <div style={{ fontFamily: DATA, fontSize: 9.5, fontWeight: 700, color: r.tone, width: 26, textAlign: "right", flexShrink: 0 }}>{Math.round(r.pct)}%</div>
                <div style={{ fontFamily: DATA, fontSize: 11, fontWeight: 700, color: C.ink, width: 38, textAlign: "right", flexShrink: 0 }}>{hm(r.v)}</div>
              </div>))}
            <div style={{ fontFamily: DATA, fontSize: 9.5, letterSpacing: 0.6, color: C.faint, textTransform: "uppercase", marginTop: 2 }}>
        ...
        return <div style={{ marginBottom: 14 }}>{card(<div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            {weekReady && (
              <div style={{ display: "flex", gap: 3, background: "rgba(255,255,255,.04)", padding: 3, borderRadius: 9, marginTop: -8 }}>
                {["night", "week"].map((v) => (
                  <button key={v} onClick={() => setSleepView(v)} style={{ fontFamily: DATA, fontSize: 9.5, fontWeight: 700, letterSpacing: 0.8, padding: "4px 10px", borderRadius: 7, border: "none", cursor: "pointer", textTransform: "capitalize", background: sleepView === v ? C.surfaceAlt : "transparent", color: sleepView === v ? C.ink : C.faint }}>{v}</button>))}
              </div>)}
          </div>
          {sl.status === "empty" && <div style={{ fontSize: 13.5, color: C.muted, lineHeight: 1.5 }}>
          ...
          {showWeek && weekChart}
          {!showWeek && sl.status === "collecting" && <div><div style={{ fontSize: 13.5, color: C.muted }}>
            Learning your baseline {"—"} {sl.have}/{sl.need} days banked.</div>{stageBar}</div>}
          {!showWeek && sl.status === "ready" && <div>
            <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
              <span style={{ fontFamily: DISPLAY, fontSize: 30, fontWeight: 700, color: C.ink }}>{hm(sl.current)}</span>
              <span style={{ fontSize: 13, color: C.faint }}>this day</span>
              <span style={{ marginLeft: "auto", fontSize: 13.5, fontWeight: 700, color: sl.flagged ? C.avoid : C.go }}>{sl.delta >= 0 ? "+" : ""}{sl.delta} min vs your {hm(sl.baseline)}</span>
            </div>
        ...
        <div style={{ marginBottom: 14 }}>{card(
          <>
            ...
            <div style={{ display: "flex", gap: 6, marginBottom: 9, flexWrap: "wrap" }}>
              {t.steps.map((mg) => (<span key={mg} style={{ padding: "6px 13px", borderRadius: 999, fontFamily: DATA, fontSize: 12, fontWeight: 700, letterSpacing: 0.8, border: `1.5px solid ${mg === t.cur ? C.go : C.hair}`, color: mg === t.cur ? C.go : C.muted, background: mg === t.cur ? C.go + "1A" : "transparent" }}>{mg} {t.unit}</span>))}
              {t.custom && <span style={{ padding: "6px 13px", borderRadius: 999, fontFamily: DATA, fontSize: 12, fontWeight: 700, letterSpacing: 0.8, border: `1.5px solid ${C.go}`, color: C.go, background: C.go + "1A" }}>{t.cur} {t.unit} (custom)</span>}
            </div>
      ...
      <div style={{ marginBottom: 14 }}>{card(
        <>
          ...
          {[
            ...
            <div key={i} style={{ display: "flex", gap: 11, padding: "10px 0", borderBottom: i < 2 ? `1px solid ${C.hair}` : "none" }}><div style={{ width: 4, borderRadius: 3, background: col, flexShrink: 0 }} /><div><div style={{ fontSize: 14.5, fontWeight: 600, color: C.ink }}>{t}</div><div style={{ fontSize: 13, color: C.muted, marginTop: 1 }}>{d}</div></div></div>
          ))}
        </>)}</div>
      ...
      <div style={{ marginBottom: 14 }}>{card(
        <>
          ...
          <div style={{ display: "flex", gap: 14, alignItems: "baseline" }}>
            <div><div style={{ fontFamily: DISPLAY, fontSize: 23, fontWeight: 700, color: C.ink }}>{projReady ? `${recentRate.toFixed(1)} lb/wk` : "—"}</div><div style={{ fontSize: 12, color: C.faint }}>{projReady ? "recent avg" : "collecting"}</div></div>
            <div><div style={{ fontFamily: DISPLAY, fontSize: 23, fontWeight: 700, color: C.go }}>{goalDate ? fmtDate(goalDate) : "—"}</div><div style={{ fontSize: 12, color: C.faint }}>{weeksToGoal ? `${goalWeight} lb in ~${weeksToGoal} wks` : "log more to project"}</div></div>
          </div>
          {goalDate ? lineChart(projection(curWeight, goalWeight, recentRate), { color: C.violet, goal: goalWeight, goalLabel: `${goalWeight}`, dashed: true }, C) : <div style={{ padding: "22px 0", textAlign: "center", color: C.faint, fontSize: 14, lineHeight: 1.5 }}>Collecting — log ~2 weeks of weigh-ins for a real projection.<br/>A rate needs at least 4 weigh-ins across 12+ days.</div>}
        </>)}</div>
        ...
        <div style={{ marginBottom: 14 }}>{card(
          <>
            ...
            <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
              <span style={{ fontFamily: DISPLAY, fontSize: 35, fontWeight: 700, color: C.violet, fontVariantNumeric: "tabular-nums" }}>{fatCorrelation.hits}/{fatCorrelation.total}</span>
              <span style={{ fontSize: 14.5, color: C.ink2, fontWeight: 500, lineHeight: 1.3 }}>of your nausea flares<br />followed a high-fat meal</span>
            </div>
            ...
            <div style={{ marginTop: 12 }}>
              {nauseaWithMeal.slice(0, 3).map((x, i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderTop: i > 0 ? `1px solid ${C.hair}` : "none" }}>
                  <div><div style={{ fontSize: 14, fontWeight: 600, color: C.ink }}>{x.meal.name}</div><div style={{ fontSize: 12, color: C.faint }}>{fmtDate(x.meal.date)} · nausea followed</div></div>
                  <span style={{ fontSize: 12.5, fontWeight: 700, color: x.meal.fat >= 30 ? C.avoid : C.go, background: x.meal.fat >= 30 ? C.avoidSoft : C.goSoft, borderRadius: 20, padding: "3px 10px" }}>{x.meal.fat}g fat</span>
                </div>
              ))}
            </div>
            <div style={{ fontSize: 11.5, color: C.faint, marginTop: 10 }}>Correlation, not proof — the more you log, the sharper this gets. No competitor connects symptoms to meals.</div>
          </>)}</div>
      )}
      ...
      {card(
        <>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
            ...
            <button onClick={addSideEffect} style={{ background: C.go, color: C.bg, border: "none", borderRadius: 999, padding: "0 18px", fontFamily: DATA, fontWeight: 800, fontSize: 12.5, letterSpacing: 1, textTransform: "uppercase", cursor: "pointer", boxShadow: `0 4px 14px ${C.go}44` }}>Log</button>
          </div>
          {[...glp.sideEffects].reverse().map((s) => (
            <div key={s.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 0", borderBottom: `1px solid ${C.hair}` }}>
              <div><span style={{ fontSize: 14.5, fontWeight: 600, color: C.ink }}>{s.symptom}</span><span style={{ fontSize: 13, color: C.faint, marginLeft: 8 }}>{fmtDate(s.date)}{s.at ? " " + new Date(s.at).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }) + (() => { const dl9 = (glp.doseLog || []).map((d9) => new Date((d9.at || d9.date + "T09:00:00")).getTime()).filter((t9) => t9 <= new Date(s.at).getTime()); if (!dl9.length) return ""; const h9 = Math.round((new Date(s.at).getTime() - Math.max(...dl9)) / 3600000); return h9 >= 0 && h9 < 200 ? " · " + h9 + " h post-dose" : ""; })() : ""}</span></div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontFamily: DATA, fontSize: 11, fontWeight: 700, letterSpacing: 0.8, textTransform: "uppercase", color: [C.go, C.caution, C.avoid][s.severity - 1], background: [C.goSoft, C.cautionSoft, C.avoidSoft][s.severity - 1], borderRadius: 999, padding: "4px 11px" }}>{["Mild", "Moderate", "Severe"][s.severity - 1]}</span>
                <button onClick={() => setGlp((g) => ({ ...g, sideEffects: g.sideEffects.filter((x) => x.id !== s.id) }))} style={{ background: "none", border: "none", color: C.faint, fontSize: 16, cursor: "pointer", padding: 4 }}>✕</button>
              </div>
            </div>
          ))}
          <div style={{ fontSize: 11.5, color: C.faint, marginTop: 10, lineHeight: 1.4 }}>Not medical advice. Severe or persistent symptoms — contact your prescriber. This log is designed to export for clinic visits.</div>
        </>, {}, (() => {
    ...
    <div style={{ position: "fixed", top: "calc(52px + env(safe-area-inset-top, 0px))", bottom: "calc(66px + env(safe-area-inset-bottom, 0px))", left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: 430, display: "flex", flexDirection: "column", overflow: "hidden", background: C.bg, zIndex: 10 }}>
      ...
      {(() => { const hd = (healthSync && healthSync.days) || [];
        ...
        return (<div style={{ padding: "0 18px 4px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
            <span style={{ fontFamily: DATA, fontSize: 10, fontWeight: 700, letterSpacing: 1.4, textTransform: "uppercase", color: C.faint }}>Reading from</span>
          </div>
          <div style={{ display: "flex", gap: 7 }}>
            {cells.map(([l, v]) => (<div key={l} style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontFamily: DATA, fontSize: 9.5, letterSpacing: 1, color: C.faint, textTransform: "uppercase" }}>{l}</div>
              <div style={{ fontFamily: DATA, fontSize: 14, fontWeight: 700, color: v ? C.ink : C.faint, marginTop: 2, display: "flex", alignItems: "center", gap: 4 }}>
      ...
      {coachMsgs.length <= 1 && (
        <div style={{ padding: "10px 18px 2px" }}>
          <div style={{ fontFamily: DATA, fontSize: 10, fontWeight: 700, letterSpacing: 1.4, textTransform: "uppercase", color: C.faint, marginBottom: 7 }}>What it actually does</div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {["Plans your week", "Ranks nearby spots", "Proposes orders", "Writes lists & tips"].map((t) => (
              <span key={t} style={{ fontFamily: DATA, fontSize: 10, fontWeight: 600, letterSpacing: 0.6, borderRadius: 999, padding: "4px 9px", color: C.go, border: `1px solid ${C.go}44`, background: C.go + "12" }}>{t}</span>))}
            <span style={{ fontFamily: DATA, fontSize: 10, fontWeight: 600, letterSpacing: 0.6, borderRadius: 999, padding: "4px 9px", color: C.avoid, border: `1px solid ${C.avoid}44`, background: C.avoid + "12" }}>✕ Give medical advice</span>
          </div>
          <div style={{ fontSize: 12, color: C.faint, marginTop: 8, lineHeight: 1.45 }}>Your prescriber owns your numbers. Bring these screens to that conversation — the report prints them.</div>
        </div>)}
      ...
      <div style={{ padding: "10px 14px", borderTop: `1px solid ${C.hair}`, background: C.surfaceAlt }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={sendCoach} disabled={coachLoading} style={{ background: C.go, color: C.surface, border: "none", borderRadius: 22, width: 46, fontSize: 19, cursor: "pointer", opacity: coachLoading ? 0.6 : 1 }}>↑</button>
        </div>
      </div>
    </div>
  );
  ...
  function renderPlan() {
    ...
    if (planView === "setup" || !mealPlan) return (
      <div>
        ...
        {onMed && <div style={{ background: C.violet + "1A", borderLeft: `4px solid ${C.violet}`, borderRadius: 12, padding: "12px 14px", marginBottom: 12 }}>
          ...
          <div style={{ fontSize: 14.5, color: C.ink, lineHeight: 1.45 }}><b>Shot {doseDayName}</b> — that day and the next run lean, bland, small-volume with eased targets. The week still averages your goal.{escalating ? " Dose-increase weeks ease further." : ""}</div>
        </div>}
        {card(<div>
          {sectionTitle("Your daily targets · from Body")}
          <div style={{ display: "flex", gap: 16 }}>
            <div><span style={{ fontSize: 21, fontWeight: 800, color: C.go }}>{targets.calories.toLocaleString()}</span><span style={{ fontSize: 13.5, color: C.muted, marginLeft: 4 }}>cal</span></div>
            <div><span style={{ fontSize: 21, fontWeight: 800, color: C.go }}>{targets.protein}g</span><span style={{ fontSize: 13.5, color: C.muted, marginLeft: 4 }}>protein</span></div>
            <div><span style={{ fontSize: 21, fontWeight: 800, color: C.ink }}>~{targets.carbs}g</span><span style={{ fontSize: 13.5, color: C.muted, marginLeft: 4 }}>carbs</span></div>
            <div><span style={{ fontSize: 21, fontWeight: 800, color: C.ink }}>≤{targets.fat}g</span><span style={{ fontSize: 13.5, color: C.muted, marginLeft: 4 }}>fat</span></div>
          </div>
        </div>, { marginBottom: 12 })}
        {card(<div>
          {sectionTitle("Prep length")}
          <div style={{ display: "flex", background: C.surfaceAlt, borderRadius: 11, padding: 3, marginBottom: 14 }}>
            {[3, 5, 7].map((n) => (<button key={n} onClick={() => setPlanDaysN(n)} style={{ flex: 1, border: "none", cursor: "pointer", padding: "8px 0", borderRadius: 9, fontFamily: BODY, fontSize: 14.5, fontWeight: 700, background: planDaysN === n ? C.surface : "transparent", color: planDaysN === n ? C.ink : C.muted }}>{n} days</button>))}
          </div>
          {sectionTitle("Meals per day")}
          <div style={{ display: "flex", background: C.surfaceAlt, borderRadius: 11, padding: 3 }}>
            {[3, 4, 5].map((n) => (<button key={n} onClick={() => { setPlanMealCount(n); setPlanMealsOn(_slotsFor(n)); }} style={{ flex: 1, border: "none", cursor: "pointer", padding: "8px 0", borderRadius: 9, fontFamily: BODY, fontSize: 14.5, fontWeight: 700, background: planMealCount === n ? C.surface : "transparent", color: planMealCount === n ? C.ink : C.muted }}>{n} meals</button>))}
          </div>
        ...
        {allergies.length > 0 && <div style={{ fontSize: 13.5, color: C.avoid, marginBottom: 12 }}><b>Filtering out:</b> {allergies.join(", ")} — hidden from every meal, same as ordering.</div>}
        <button onClick={() => generatePlan(false)} disabled={!!planBusy} style={{ width: "100%", background: C.go, color: C.surface, border: "none", borderRadius: 12, padding: "14px 0", fontFamily: BODY, fontSize: 16, fontWeight: 800, cursor: "pointer", opacity: planBusy ? 0.6 : 1 }}>{busyLabel || (mealPlan ? "Regenerate my week →" : "Generate my week →")}</button>
        {mealPlan && !planBusy && <button onClick={() => setPlanView("week")} style={{ width: "100%", background: "none", border: "none", color: C.muted, fontFamily: BODY, fontSize: 14, marginTop: 10, cursor: "pointer", textDecoration: "underline" }}>Back to current plan</button>}
        {planErr && <div style={{ fontSize: 13.5, color: C.avoid, marginTop: 10 }}>Plan hiccup: {planErr}</div>}
        {planNote && <div style={{ fontSize: 12.5, color: C.muted, marginTop: 8, lineHeight: 1.5 }}>{planNote}</div>}
    ...
    if (planView === "grocery") { const done = mealPlan.grocery.filter((g) => g.checked).length; const sections = [...new Set(mealPlan.grocery.map((g) => g.section))];
      ...
      <div>
        <button onClick={() => setPlanView("week")} style={{ background: "none", border: "none", color: C.go, fontFamily: BODY, fontSize: 14.5, fontWeight: 700, cursor: "pointer", padding: 0, marginBottom: 10 }}>← Your week</button>
        {sectionTitle(`Grocery · ${mealPlan.days.length} days`)}
        {card(<div>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14, marginBottom: 8 }}><b style={{ color: C.ink }}>{done} of {mealPlan.grocery.length} gathered</b><span style={{ color: C.muted }}>consolidated across all meals</span></div>
          ...
          {(() => { const est = groceryEst;
            if (!est.matched) return (<div style={{ fontSize: 12, color: C.faint, marginTop: 9, lineHeight: 1.45 }}>No cost estimate yet — prices you save in Shop Mode build a price book per store, and this totals only what you have actually paid.</div>);
            return (<div style={{ marginTop: 9 }}>
              {est.stores.map((st) => (
                <div key={st.store} style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 8, padding: "3px 0" }}>
                  <span style={{ fontSize: 13.5, color: C.ink, fontWeight: 700, textTransform: "capitalize" }}>{st.store}{est.cheapest && est.cheapest.store === st.store ? <span style={{ color: C.go, fontWeight: 700 }}> · cheapest</span> : ""}</span>
                  <span style={{ fontSize: 14, color: C.ink, fontWeight: 800, whiteSpace: "nowrap" }}>≈ ${st.total.toFixed(2)} <span style={{ fontSize: 12, color: C.muted, fontWeight: 500 }}>· {st.matched}/{mealPlan.grocery.length}</span></span>
                </div>))}
          ...
          {groceryNote && <div style={{ fontSize: 12.5, color: C.muted, marginTop: 7 }}>Built-in package math shown — AI consolidation hiccup: {groceryNote}</div>}
        </div>, { marginBottom: 12 })}
        {sections.map((sec) => card(<div key={sec}>
          ...
          {mealPlan.grocery.map((g, i) => g.section !== sec ? null : (
            <div key={i} onClick={() => toggleGroceryItem(i)} style={{ display: "flex", alignItems: "center", gap: 11, padding: "9px 0", borderTop: `1px solid ${C.hair}`, cursor: "pointer" }}>
              <div style={{ width: 21, height: 21, borderRadius: 6, flexShrink: 0, border: `2px solid ${g.checked ? C.go : C.hair}`, background: g.checked ? C.go : "transparent", display: "flex", alignItems: "center", justifyContent: "center", color: C.surface, fontSize: 14, fontWeight: 800 }}>{g.checked ? "✓" : ""}</div>
        ...
        {card(<div>
          ...
          {shopScan.status !== "found" && (camOn && camFor === "shop" ? (
            <div style={{ borderRadius: 14, overflow: "hidden", position: "relative", marginBottom: 10, background: "#000" }}>
              ...
              <button onClick={stopCam} style={{ position: "absolute", top: 10, right: 10, background: "rgba(0,0,0,0.6)", color: "#fff", border: "none", borderRadius: 20, padding: "6px 13px", fontFamily: BODY, fontSize: 13.5, fontWeight: 700, cursor: "pointer" }}>Stop</button>
              <div style={{ position: "absolute", bottom: 8, left: 0, right: 0, textAlign: "center", color: "rgba(255,255,255,0.9)", fontSize: 12.5, fontWeight: 600, textShadow: "0 1px 3px rgba(0,0,0,0.8)" }}>Center the shelf item's barcode in the box</div>
            </div>
          ) : (
            <button onClick={() => startCam("shop")} style={{ width: "100%", borderRadius: 14, border: `1.5px dashed ${C.go}88`, background: C.goSoft, padding: "16px 14px", textAlign: "center", marginBottom: 10, cursor: "pointer" }}>
              ...
              <div style={{ fontFamily: BODY, fontSize: 12.5, color: C.muted, marginTop: 2 }}>Point at the shelf item's barcode — or type it below</div>
            </button>
          ))}
          {shopScan.status !== "found" && <div style={{ display: "flex", gap: 8 }}>
            ...
            <button onClick={shopLookup} disabled={shopScan.status === "loading"} style={{ background: C.go, color: C.surface, border: "none", borderRadius: 10, padding: "0 16px", fontFamily: BODY, fontWeight: 700, fontSize: 14.5, cursor: "pointer", opacity: shopScan.status === "loading" ? 0.6 : 1 }}>{shopScan.status === "loading" ? "…" : "Check"}</button>
          </div>}
          {shopScan.status === "miss" && <div style={{ fontSize: 13.5, color: C.avoid, marginTop: 8 }}>Not found in Open Food Facts / USDA — log it by name from Today instead.</div>}
          {shopScan.status === "found" && (() => { const f = shopScan.food; const dense = f.calories > 0 && f.protein * 4 >= f.calories * 0.22; const mi = shopListMatch(); return (
            <div>
              <div style={{ fontSize: 15.5, fontWeight: 700, color: C.ink }}>{f.name}{f.brand ? <span style={{ color: C.muted, fontWeight: 500 }}> · {f.brand}</span> : null}</div>
              <div style={{ fontSize: 13.5, color: C.muted, margin: "3px 0 7px" }}>per {f.basis}: <b style={{ color: C.go }}>{f.protein}g protein</b> · {f.calories} cal · {f.carbs}g carb · {f.fat}g fat</div>
              {dense ? <div style={{ fontSize: 13.5, color: C.go, fontWeight: 700 }}>✓ Protein-dense — fits the plan</div> : <div style={{ fontSize: 13.5, color: C.caution, fontWeight: 700 }}>Light on protein for its calories{f.fat >= 15 ? " · higher fat — go slow" : ""}</div>}
              {mi >= 0 && !mealPlan.grocery[mi].checked && <button onClick={() => toggleGroceryItem(mi)} style={{ marginTop: 9, width: "100%", background: C.surfaceAlt, border: `1.5px solid ${C.go}`, color: C.go, borderRadius: 10, padding: "10px 0", fontFamily: BODY, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>✓ Check off "{mealPlan.grocery[mi].item}"</button>}
              <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                <input value={shopPrice} onChange={(e) => setShopPrice(e.target.value)} placeholder="$ price" inputMode="decimal" style={{ width: 90, background: C.surfaceAlt, border: `1px solid ${C.hair}`, borderRadius: 10, padding: "10px 11px", color: C.ink, fontFamily: BODY, fontSize: 14.5, outline: "none" }} />
                <input value={shopStore} onChange={(e) => setShopStore(e.target.value)} placeholder="Store" style={{ flex: 1, background: C.surfaceAlt, border: `1px solid ${C.hair}`, borderRadius: 10, padding: "10px 11px", color: C.ink, fontFamily: BODY, fontSize: 14.5, outline: "none" }} />
                <button onClick={rememberShopPrice} style={{ background: C.go, color: C.bg, border: "none", borderRadius: 999, padding: "0 15px", fontFamily: DATA, fontWeight: 800, fontSize: 12, letterSpacing: 1, textTransform: "uppercase", cursor: "pointer" }}>Save</button>
              </div>
          ...
          <div style={{ fontSize: 12.5, color: C.faint, marginTop: 9 }}>{priceLog.length > 0 ? `${priceLog.length} price${priceLog.length > 1 ? "s" : ""} remembered — your per-store price book grows with every trip.` : "Type the numbers under any barcode. Prices you save build your per-store price book."}</div>
        </div>, { marginBottom: 12 })}
      </div>
    ); }
    if (planView === "meal") { const [di, si] = planMealRef; const day = mealPlan.days[di]; const slot = day && day.slots[si]; if (!slot) { setPlanView("week"); return null; } const img = slot.photo || slot.image; return (
      <div>
        <button onClick={() => setPlanView("week")} style={{ background: "none", border: "none", color: C.go, fontFamily: BODY, fontSize: 14.5, fontWeight: 700, cursor: "pointer", padding: 0, marginBottom: 10 }}>← {day.label} · Your week</button>
        <div style={{ borderRadius: 16, overflow: "hidden", position: "relative", height: 170, marginBottom: 12, background: C.surfaceAlt }}>
          {img ? <img src={img} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", color: C.faint, fontSize: 14, fontFamily: BODY }}>No photo yet — yours goes here</div>}
          <label style={{ position: "absolute", left: 0, right: 0, bottom: 0, background: "linear-gradient(transparent, rgba(0,0,0,.55))", color: "#fff", fontSize: 13.5, fontWeight: 700, fontFamily: BODY, padding: "20px 12px 9px", cursor: "pointer" }}>
        ...
        <div style={{ fontSize: 12.5, fontWeight: 800, letterSpacing: 0.8, color: (day.dose || day.after) ? C.violet : C.muted, textTransform: "uppercase" }}>{day.label} {slot.slot === "snack2" ? "snack" : slot.slot}{(day.dose || day.after) ? " · gentle menu" : ""}</div>
        <div style={{ fontFamily: DISPLAY, fontSize: 22, fontWeight: 800, color: C.ink, margin: "4px 0 5px", lineHeight: 1.2 }}>{slot.name}</div>
        ...
        {(day.dose || day.after) && <div style={{ background: C.violet + "1A", borderLeft: `4px solid ${C.violet}`, borderRadius: 12, padding: "11px 13px", marginBottom: 12, fontSize: 14, color: C.ink, lineHeight: 1.45 }}><b style={{ color: C.violet }}>Why this meal today:</b> post-shot, warm bland low-fat food is easiest to keep down. Eat slowly — stop at comfortable, not full.</div>}
        {slot.ingredients.length > 0 && card(<div>{sectionTitle("Ingredients · on your grocery list")}{slot.ingredients.map((x, i) => <div key={i} style={{ fontSize: 14.5, color: C.ink, padding: "6px 0", borderTop: i ? `1px solid ${C.hair}` : "none" }}>{x}</div>)}</div>, { marginBottom: 12 })}
        {slot.steps.length > 0 && card(<div>{sectionTitle("Steps")}{slot.steps.map((x, i) => <div key={i} style={{ display: "flex", gap: 9, padding: "6px 0", borderTop: i ? `1px solid ${C.hair}` : "none" }}><b style={{ color: C.go, fontSize: 14 }}>{i + 1}</b><span style={{ fontSize: 14.5, color: C.ink, lineHeight: 1.45 }}>{x}</span></div>)}</div>, { marginBottom: 12 })}
        {slot.url && <a href={slot.url} target="_blank" rel="noreferrer" style={{ display: "block", fontSize: 12.5, color: C.faint, marginBottom: 12, textDecoration: "underline" }}>Original recipe source ↗</a>}
        <button onClick={() => logPlannedMeal(di, si)} style={{ width: "100%", background: slot.logged ? C.surfaceAlt : C.go, color: slot.logged ? C.go : C.bg, border: slot.logged ? `1.5px solid ${C.go}` : "none", borderRadius: 12, padding: "14px 0", fontFamily: DATA, fontSize: 14, fontWeight: 800, letterSpacing: 1.2, textTransform: "uppercase", cursor: "pointer" }}>{slot.logged ? "Logged ✓" : "Log this meal ✓"}</button>
        ...
        {swapList !== null && !swapBusy && (
          <div style={{ marginTop: 10 }}>
            {swapList.length === 0 ? (
              ...
              <div style={{ fontFamily: DATA, fontSize: 10, fontWeight: 700, letterSpacing: 1.4, textTransform: "uppercase", color: C.faint, marginBottom: 7 }}>Matched to this slot</div>
              {swapList.map((r, i) => (
                <div key={i} onClick={() => applySwap(di, si, r)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, background: C.surface, border: `1px solid ${C.hair}`, borderRadius: 14, padding: "11px 13px", marginBottom: 8, cursor: "pointer" }}>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: 14.5, fontWeight: 700, color: C.ink, lineHeight: 1.25 }}>{r.name}</div>
                    <div style={{ fontFamily: DATA, fontSize: 11.5, color: C.faint, marginTop: 3 }}>{Math.round(r.p || 0)} P · {Math.round(r.cal || 0)} cal · {(r.ingredients || []).length} ingredients</div>
                  </div>
                  <span style={{ fontFamily: DATA, fontSize: 10, fontWeight: 700, letterSpacing: 1, color: C.go, border: `1px solid ${C.go}55`, borderRadius: 999, padding: "4px 10px", flexShrink: 0 }}>Swap</span>
                </div>))}
              <div style={{ fontSize: 11.5, color: C.faint, lineHeight: 1.45 }}>Candidates match this slot's protein and calories and skip your allergies. Swapping keeps the rest of the week untouched.</div>
            </>)}
          </div>)}
      </div>
    ); }
    ...
    return (
      <div>
        ...
        {onMed && <div style={{ fontSize: 13.5, fontWeight: 700, color: C.go, marginBottom: 10 }}>✓ Synced to your dose calendar — shot {doseDayName}</div>}
        <div style={{ display: "flex", gap: 5, marginBottom: 12 }}>
          {mealPlan.days.map((w, i) => { const on = i === Math.min(planSel, mealPlan.days.length - 1); const dd = w.dose || w.after; return (
            <button key={i} onClick={() => setPlanSel(i)} style={{ flex: 1, border: `1.5px solid ${on ? "transparent" : dd ? C.violet + "55" : C.hair}`, cursor: "pointer", borderRadius: 12, padding: "8px 0 6px", fontFamily: BODY, background: on ? (dd ? C.violet : C.go) : dd ? C.violet + "14" : C.surface }}>
              ...
              <div style={{ fontSize: 10.5, marginTop: 2, color: on ? C.surface : dd ? C.violet : C.faint }}>{w.dose ? "💉" : w.after ? "·💉" : "·"}</div>
            </button>
          ); })}
        </div>
        {card(<div>
          ...
          <div key={si} onClick={() => { setPlanMealRef([Math.min(planSel, mealPlan.days.length - 1), si]); setPlanView("meal"); }} style={{ background: C.surface, border: `1px solid ${si === tonightIdx ? C.go + "66" : C.hair}`, borderRadius: 16, display: "flex", alignItems: "center", gap: 11, padding: "11px 12px", marginBottom: 10, cursor: "pointer" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <div style={{ fontSize: 11.5, fontWeight: 800, letterSpacing: 0.8, color: (day.dose || day.after) ? C.violet : C.muted, textTransform: "uppercase" }}>{slot.slot === "snack2" ? "snack" : slot.slot}</div>
                {si === tonightIdx && <span style={{ fontFamily: DATA, fontSize: 10, fontWeight: 700, letterSpacing: 1, borderRadius: 999, padding: "2px 7px", color: C.go, border: `1px solid ${C.go}55`, background: C.go + "1A" }}>TONIGHT</span>}
              </div>
              <div style={{ fontSize: 15.5, fontWeight: 700, color: C.ink, lineHeight: 1.25 }}>{slot.name}</div>
            </div>
            <div style={{ textAlign: "right", flexShrink: 0 }}>
              <div style={{ fontSize: 20, fontWeight: 800, color: C.go, lineHeight: 1 }}>{Math.round(slot.perServing.protein * slot.servings)}g</div>
              <div style={{ fontSize: 12.5, color: C.muted, marginTop: 2 }}>{Math.round(slot.perServing.calories * slot.servings)} cal · {Math.round((slot.perServing.carbs || 0) * slot.servings)}g carb · {Math.round((slot.perServing.fat || 0) * slot.servings)}g fat</div>
        ...
        {!planBusy && <button onClick={() => generatePlan(true)} style={{ width: "100%", background: "none", border: "none", color: C.muted, fontFamily: BODY, fontSize: 13.5, margin: "2px 0 10px", cursor: "pointer", textDecoration: "underline" }}>↻ Fresh ideas — re-search recipes (uses API quota)</button>}
        <div style={{ display: "flex", gap: 9 }}>
          <button onClick={() => setPlanView("setup")} style={{ flex: 1, background: C.surface, border: `1px solid ${C.hair}`, borderRadius: 12, padding: "12px 0", fontFamily: BODY, fontSize: 14.5, fontWeight: 700, color: C.ink, cursor: "pointer" }}>Plan settings</button>
          <button onClick={() => setPlanView("grocery")} style={{ flex: 1.3, background: C.go, border: "none", borderRadius: 12, padding: "12px 0", fontFamily: BODY, fontSize: 14.5, fontWeight: 800, color: C.surface, cursor: "pointer" }}>Grocery list →</button>
        </div>
      </div>
    );
  }
      ...
      <div style={{ width: "100%", maxWidth: 430, background: C.bg, minHeight: "100vh", position: "relative", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ position: "sticky", top: 0, zIndex: 45, height: "calc(52px + env(safe-area-inset-top, 0px))", paddingTop: "env(safe-area-inset-top, 0px)", paddingLeft: 16, paddingRight: 16, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: `1px solid ${C.hair}`, background: C.bg }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ fontFamily: DISPLAY, fontSize: 19, fontWeight: 700, color: C.ink, letterSpacing: -0.3 }}>ForkCaster</span>
            {appVer && <span style={{ fontSize: 11, color: C.faint, fontWeight: 600, marginTop: 4 }}>v{appVer}</span>}
          </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", paddingBottom: "calc(88px + env(safe-area-inset-bottom, 0px))" }}>
          ...
          {tab === "body" && <div>{(() => {
            ...
            return card(<div>
              ...
              {ct.status !== "ok" ? (
                <div style={{ fontSize: 13.5, color: C.muted, lineHeight: 1.55 }}>Set a goal weight and {ct.reason} — then this turns your forecast picture into the numbers that get you there.</div>
              ) : (<div>
                <div style={{ fontSize: 14.5, color: C.ink, fontWeight: 700, lineHeight: 1.5 }}>{ct.line}</div>
                ...
                <div style={{ marginTop: 12 }}>
                  {contractScorecard({ mealLog, targets, workoutLog, adaptive: adaptiveRead(weightLog, hd0, glp, 0) || {}, contract: ct, todayISO: todayISO() }).map((r, i) => (
                    <div key={r.key} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "7px 0", borderTop: i ? `1px solid ${C.hair}` : `1px solid ${C.hair}` }}>
                      <span style={{ fontSize: 13.5, color: C.ink }}>{r.unknown ? "○" : r.ok ? "✓" : "✕"} {r.label}</span>
                      <span style={{ fontSize: 13, color: r.unknown ? C.faint : r.ok ? C.go : C.caution, fontWeight: 700 }}>{r.detail}</span>
                    </div>))}
                </div>
                <div style={{ fontSize: 11.5, color: C.faint, marginTop: 8, lineHeight: 1.45 }}>The picture is a visualization, not a promise — lighting and genetics are not macros. These four levers are the part you control.</div>
              </div>)}
            </div>, { marginBottom: 12 });
            ...
            return card(<div>
              ...
              {hd.length === 0 ? (
                <div style={{ fontSize: 13.5, color: C.muted, lineHeight: 1.55 }}>
                  ...
                  <div style={{ background: C.surfaceAlt, borderRadius: 8, padding: "8px 10px", margin: "6px 0", fontSize: 12, wordBreak: "break-all", color: C.ink }}>{`${window.location.origin}/api/health/sync?token=${healthSync ? healthSync.token : "…"}`}</div>
                  <div>5. Automation → Time of Day → daily → Run Immediately → your shortcut.</div>
                  <div style={{ marginTop: 6, color: C.faint, fontSize: 12.5 }}>Prefer an app? The Health Auto Export app posting to the same URL also works. Manual weigh-ins here stay authoritative either way.</div>
                </div>
              ) : (
                <div>
                  ...
                  <div style={{ display: "flex", gap: 14, marginTop: 8 }}>
                    <button onClick={async () => { if (!window.confirm("Clear all synced Apple Health data on your node? Steps, weight, workouts and body composition from the phone will be wiped. Your sync setup keeps working — the next sync refills it.")) return;
                      ...
                      style={{ background: "none", border: "none", color: C.caution, fontFamily: BODY, fontSize: 12.5, fontWeight: 700, cursor: "pointer", padding: 0, textDecoration: "underline" }}>Clear synced data</button>
                    <button onClick={() => setPrefs({ ...prefs, hideHealthCard: true })} style={{ background: "none", border: "none", color: C.muted, fontFamily: BODY, fontSize: 12.5, fontWeight: 700, cursor: "pointer", padding: 0, textDecoration: "underline" }}>Hide this card</button>
                  </div>
                </div>
              )}
            </div>, { marginBottom: 12 });
              ...
              <div style={{ display: "flex", gap: 10, marginBottom: 9 }}>
                {[["Formula", "trend vs dose"], ["Measured", "your weigh-ins"], ["Confidence", "grows weekly"]].map(([l6, v6]) => (
                  <div key={l6} style={{ flex: 1 }}>
                    <div style={{ fontFamily: DATA, fontSize: 10, letterSpacing: 1.1, color: C.faint, textTransform: "uppercase" }}>{l6}</div>
                    <div style={{ fontFamily: DATA, fontSize: 11.5, fontWeight: 700, color: C.ink2, marginTop: 3 }}>{v6}</div>
                  </div>))}
              </div>
              {ar.status === "collecting" ? (
                <div style={{ fontSize: 13.5, color: C.muted, lineHeight: 1.5 }}>Watching quietly — {ar.pts}/4 weigh-ins across {ar.spanDays}/12 days so far. Log weight every few days (or sync a scale) and this wakes up with a verdict on your real loss rate, dose-aware.</div>
              ) : (
                <div>
                  <div style={{ fontSize: 14, color: col, fontWeight: 700, lineHeight: 1.45 }}>{ar.detail}</div>
                  ...
                  {applied && <div style={{ fontSize: 12.5, color: C.go, marginTop: 7, fontWeight: 700 }}>✓ Applied today — adjust anytime in Plan settings.</div>}
                  <div style={{ fontSize: 11.5, color: C.faint, marginTop: 7 }}>{ar.pts} weigh-ins over {ar.spanDays} days · a suggestion, not a prescription — your prescriber owns your targets</div>
                </div>
              )}
            </div>, { marginBottom: 12 });
          })()}{(() => {
            ...
            return card(<div>
              ...
              {dr.status === "collecting" ? (
                ...
                <div style={{ fontSize: 13.5, color: C.muted, lineHeight: 1.55 }}>No clear fat↔symptom pattern in {dr.days} logged days ({dr.inWin} in dose windows) — either good tolerance or thin data. Keep logging; the picture sharpens itself.</div>
              ) : (
                <div>
                  <div style={{ fontSize: 14, color: C.ink, fontWeight: 700, lineHeight: 1.5 }}>Meals over ~{dr.ceiling}g fat {dr.scope} preceded GI symptoms {dr.aboveSym} of {dr.above} times — at or under, {dr.belowSym} of {dr.below}.</div>
                  <div style={{ fontSize: 13.5, color: C.go, fontWeight: 700, marginTop: 7 }}>Your personal fat ceiling: ~{dr.ceiling}g (generic default: 15g). "Order for me" now uses YOUR number during dose windows.</div>
                  <div style={{ fontSize: 11.5, color: C.faint, marginTop: 7 }}>Correlation from your own {dr.days} logged days — a pattern, not proof. Worth showing your prescriber.</div>
                </div>
              )}
            </div>, { marginBottom: 12 });
          })()}{renderBody()}</div>}
          ...
          {splashOn && (
            <div style={{ position: "fixed", inset: 0, zIndex: 400, background: "#050807", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", opacity: splashFade ? 0 : 1, transition: "opacity .45s ease", pointerEvents: splashFade ? "none" : "auto" }}>
              ...
              <div className="fc-lockup">
                ...
                <div style={{ fontFamily: DISPLAY, fontSize: 30, fontWeight: 700, color: "#f2f7f4", marginTop: 24, letterSpacing: -0.3 }}>Fork<span style={{ color: "#63D48C" }}>Caster</span></div>
                <div style={{ fontFamily: BODY, fontSize: 14.5, color: "#94a89c", marginTop: 10 }}>Your best body is in the <span style={{ color: "#63D48C" }}>forecast</span>.</div>
              </div>
            </div>
          )}
          {infoOpen && (
            <div onClick={() => setInfoOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(30,20,10,0.5)", zIndex: 90, display: "flex", alignItems: "flex-end" }}>
              <div onClick={(e) => e.stopPropagation()} style={{ background: C.surface, borderRadius: "22px 22px 0 0", width: "100%", maxHeight: "86vh", overflowY: "auto", padding: "20px 20px calc(24px + env(safe-area-inset-bottom, 0px))" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                  <div style={{ fontFamily: DISPLAY, fontSize: 22, fontWeight: 800, color: C.ink }}>How ForkCaster gets its numbers</div>
                ...
                <div style={{ fontSize: 13.5, color: C.muted, lineHeight: 1.5, marginBottom: 14 }}>Plain answers about what's measured, what's estimated, and how much to trust each number. Everything below runs on your own server — your data never leaves it.</div>
                {[
                  ...
                  <div key={title} style={{ marginBottom: 14 }}>
                    <div style={{ fontSize: 12.5, fontWeight: 800, letterSpacing: 0.6, textTransform: "uppercase", color: C.go, marginBottom: 6 }}>{title}</div>
                    {rows.map(([h, b]) => (
                      <div key={h} style={{ marginBottom: 7 }}>
                        <span style={{ fontSize: 14, fontWeight: 700, color: C.ink }}>{h} — </span>
        ...
        {sheetCard && (
          <div style={{ position: "fixed", inset: 0, background: C.bg, zIndex: 70, overflowY: "auto", WebkitOverflowScrolling: "touch" }}>
            <div style={{ position: "sticky", top: 0, background: C.bg, borderBottom: `1px solid ${C.hair}`, display: "flex", alignItems: "center", gap: 10, padding: "calc(12px + env(safe-area-inset-top, 0px)) 13px 12px", zIndex: 2 }}>
              <button onClick={() => setSheetCard(null)} style={{ width: 30, height: 30, borderRadius: 999, background: C.surfaceAlt, border: "none", color: C.ink2, fontSize: 18, lineHeight: 1, cursor: "pointer", flexShrink: 0 }}>{"‹"}</button>
              <span style={{ fontFamily: DATA, fontSize: 11.5, fontWeight: 800, letterSpacing: 1.3, textTransform: "uppercase", color: sheetCard.color || C.ink }}>{sheetCard.title}</span>
          ...
          <div onClick={() => setLogOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 60, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
            <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 430, background: C.surface, borderRadius: "22px 22px 0 0", padding: "20px 20px calc(28px + env(safe-area-inset-bottom, 0px))", maxHeight: "82vh", overflowY: "auto" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                <div style={{ fontFamily: DATA, fontSize: 14, fontWeight: 800, letterSpacing: 1.8, textTransform: "uppercase", color: C.ink }}>Scan or log food</div>
                <button onClick={() => { stopCam(); setLogOpen(false); }} style={{ background: C.surfaceAlt, border: "none", width: 30, height: 30, borderRadius: 99, color: C.muted, fontSize: 16, cursor: "pointer" }}>✕</button>
              </div>
              ...
              {camOn && camFor === "log" ? (
                <div style={{ borderRadius: 14, overflow: "hidden", position: "relative", marginBottom: 14, background: "#000" }}>
                  ...
                  <button onClick={stopCam} style={{ position: "absolute", top: 10, right: 10, background: "rgba(0,0,0,0.6)", color: "#fff", border: "none", borderRadius: 20, padding: "6px 13px", fontFamily: BODY, fontSize: 13.5, fontWeight: 700, cursor: "pointer" }}>Stop</button>
                  <div style={{ position: "absolute", bottom: 8, left: 0, right: 0, textAlign: "center", color: "rgba(255,255,255,0.9)", fontSize: 12.5, fontWeight: 600, textShadow: "0 1px 3px rgba(0,0,0,0.8)" }}>{camHint ? "Struggling? More light, hold 4–8 inches away, keep steady — or type the number below" : "Center the barcode in the box"}</div>
                </div>
              ) : (
                <button onClick={startCam} style={{ width: "100%", borderRadius: 14, border: `1.5px dashed ${C.go}88`, background: C.goSoft, padding: "22px 16px", textAlign: "center", marginBottom: 14, cursor: "pointer" }}>
                  ...
                  <div style={{ fontFamily: DATA, fontSize: 14, fontWeight: 800, letterSpacing: 1.6, textTransform: "uppercase", color: C.go }}>Scan with camera</div>
                  <div style={{ fontFamily: BODY, fontSize: 12.5, color: C.muted, marginTop: 5 }}>Point at any food barcode — or type it below</div>
                </button>
              )}
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                <input value={barcode} onChange={(e) => setBarcode(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") lookupBarcode(barcode); }} placeholder="Barcode number" inputMode="numeric"
                  style={{ flex: 1, fontFamily: DATA, fontSize: 15.5, fontWeight: 600, color: C.ink, background: C.surfaceAlt, border: `1px solid ${C.hair}`, borderRadius: 12, padding: "12px 13px", outline: "none", boxSizing: "border-box" }} />
                <button onClick={() => lookupBarcode(barcode)} disabled={scan.status === "loading"} style={{ background: C.go, color: C.bg, border: "none", borderRadius: 999, padding: "0 18px", fontFamily: DATA, fontWeight: 800, fontSize: 12, letterSpacing: 1.1, textTransform: "uppercase", cursor: "pointer", opacity: scan.status === "loading" ? 0.6 : 1 }}>{scan.status === "loading" ? "…" : "Look up"}</button>
              </div>
              <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                <input value={foodQuery} onChange={(e) => setFoodQuery(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") searchFood(); }} placeholder="Or search food by name (e.g. grilled chicken breast)"
                  style={{ flex: 1, fontFamily: DATA, fontSize: 13.5, color: C.ink, background: C.surfaceAlt, border: `1px solid ${C.hair}`, borderRadius: 12, padding: "12px 14px", outline: "none" }} />
                <button onClick={searchFood} style={{ background: C.go, color: C.bg, border: "none", borderRadius: 999, padding: "0 16px", fontFamily: DATA, fontSize: 12, fontWeight: 800, letterSpacing: 1.1, textTransform: "uppercase", cursor: "pointer" }}>Search</button>
              </div>
              <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                <input value={nlText} onChange={(e) => setNlText(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") logByDescription(); }} placeholder={'Or describe it: "2 eggs, buttered toast, black coffee"'}
                  style={{ flex: 1, fontFamily: DATA, fontSize: 13.5, color: C.ink, background: C.surfaceAlt, border: `1px solid ${C.hair}`, borderRadius: 12, padding: "12px 14px", outline: "none" }} />
                <button onClick={logByDescription} disabled={nlBusy} style={{ background: C.violet, color: C.bg, border: "none", borderRadius: 999, padding: "0 16px", fontFamily: DATA, fontSize: 12, fontWeight: 800, letterSpacing: 1.1, textTransform: "uppercase", cursor: "pointer", opacity: nlBusy ? 0.6 : 1 }}>{nlBusy ? "…" : "Log it"}</button>
              </div>
              {foodResults === "loading" && <div style={{ fontSize: 13.5, color: C.faint, padding: "8px 2px" }}>Searching…</div>}
              {foodResults && foodResults.error && <div style={{ fontSize: 13.5, color: C.avoid, padding: "8px 2px", lineHeight: 1.45 }}>Food search couldn't reach its sources — the node may be offline or the databases are down. The barcode and describe-it paths still work.</div>}
              {Array.isArray(foodResults) && foodResults.length === 0 && <div style={{ fontSize: 13.5, color: C.muted, padding: "8px 2px" }}>No matches — try fewer words.</div>}
                  ...
                  {foodResults.map((f, i) => (
                    <div key={i} onClick={() => { setScan({ status: "found", food: f }); setFoodResults(null); setFoodQuery(""); }}
                      ...
                      <div style={{ minWidth: 0, paddingRight: 10 }}>
                        <div style={{ fontSize: 14.5, fontWeight: 700, color: C.ink, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{f.name}</div>
                        <div style={{ fontSize: 12, color: C.faint }}>{f.brand || f.source} · per {f.basis}</div>
                      </div>
                      <div style={{ textAlign: "right", flexShrink: 0 }}>
                        <div style={{ fontFamily: DISPLAY, fontSize: 16, fontWeight: 700, color: C.go }}>{f.protein}g</div>
                        <div style={{ fontSize: 11.5, color: C.faint }}>{f.calories} cal</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, margin: "4px 0 14px" }}>
                <div style={{ flex: 1, height: 1, background: C.hair }} /><span style={{ fontSize: 12, color: C.faint }}>or</span><div style={{ flex: 1, height: 1, background: C.hair }} />
              </div>
              ...
              <div style={{ fontSize: 11.5, color: C.faint, textAlign: "center", marginBottom: 14, lineHeight: 1.45 }}>Read digit-for-digit — as exact as the label itself.</div>
              ...
              <button onClick={() => setInfoOpen(true)} style={{ width: "100%", background: "none", border: "none", color: C.muted, fontFamily: BODY, fontSize: 12.5, cursor: "pointer", padding: "7px 0 0", textDecoration: "underline" }}>ⓘ What's exact vs estimated? Tap to read — how ForkCaster gets its numbers</button>

              {scan.status === "found" && (
                <div ref={resultRef} style={{ background: C.goSoft, border: `1px solid ${C.go}44`, borderRadius: 14, padding: 15, marginBottom: 12 }}>
                  <div style={{ fontSize: 16, fontWeight: 700, color: C.ink }}>{scan.food.name}</div>
                  ...
                  <div style={{ display: "flex", gap: 12, marginTop: 6, flexWrap: "wrap" }}>
                    {[["Cal", scan.food.calories], ["Protein", `${scan.food.protein}g`], ["Carbs", `${scan.food.carbs}g`], ["Fat", `${scan.food.fat}g`], ["Fiber", `${scan.food.fiber || 0}g`]].map(([l, v]) => (
                      <div key={l}><div style={{ fontFamily: DISPLAY, fontSize: 19, fontWeight: 700, color: C.ink }}>{v}</div><div style={{ fontSize: 11.5, color: C.faint }}>{l}</div></div>
                    ))}
                  </div>
                  <div style={{ fontSize: 11.5, color: C.faint, marginTop: 8 }}>per {scan.food.basis} · source: {scan.food.source || "Open Food Facts"}</div>
                  {Array.isArray(scan.food.items) && scan.food.items.length > 0 && (
                    <div style={{ marginTop: 6 }}>
                      {scan.food.items.map((it, i) => (
                        <div key={i} style={{ fontSize: 11.5, color: C.faint }}>{it.qty ? `${it.qty} ` : ""}{it.item} — {it.calories} cal</div>
                      ))}
                      {scan.food.adjusted ? <div style={{ fontSize: 11, color: C.faint, fontStyle: "italic" }}>calories cross-checked against macros</div> : null}
                    </div>
                  )}
                  <button onClick={addLoggedFood} style={{ width: "100%", marginTop: 12, background: C.go, color: C.surface, border: "none", borderRadius: 11, padding: "13px 0", fontFamily: BODY, fontSize: 15.5, fontWeight: 700, cursor: "pointer" }}>Add to today →</button>
                </div>
              )}
              {scan.status === "miss" && <div style={{ fontSize: 14, color: C.muted, padding: "4px 2px" }}>Not found in Open Food Facts, USDA, or FatSecret. Try another barcode, or use the AI photo estimate below.</div>}
              {scan.status === "error" && <div style={{ fontSize: 14, color: C.avoid, padding: "4px 2px", lineHeight: 1.45 }}>{scan.message || "Couldn't reach your node — check the connection (or log into the Umbrel dashboard once) and retry."}</div>}
            </div>
          </div>
        )}
        ...
        {simOpen && (
          <div onClick={() => setSimOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 60, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
            <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 430, maxHeight: "88dvh", overflowY: "auto", background: C.bg, borderRadius: "22px 22px 0 0", padding: "18px 18px calc(24px + env(safe-area-inset-bottom, 0px))" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                <div style={{ fontFamily: DISPLAY, fontSize: 21, fontWeight: 700, color: C.ink }}>Body Forecaster</div>
                <button onClick={() => setSimOpen(false)} style={{ background: C.surfaceAlt, border: "none", borderRadius: 16, width: 32, height: 32, color: C.muted, fontSize: 16, cursor: "pointer" }}>×</button>
              </div>
              <div style={{ fontSize: 12.5, color: C.muted, marginBottom: 12 }}>Photorealistic preview of you at {fmtWt(goalWeight, 0)} {wtU}. Your real Before/After photos stay untouched.</div>
              {[["front", "Front source", simSel, setSimSel], ["back", "Back source", simSelBack, setSimSelBack]].map(([v, lbl, sel, setSel]) => {
                ...
                return (
                  <div key={v}>
                    <div style={{ fontSize: 12, color: C.muted, textTransform: "uppercase", letterSpacing: 0.4, marginBottom: 6 }}>{lbl}{v === "back" ? " · optional" : ""}</div>
                    <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 4, marginBottom: 12 }}>
                      {v === "back" && <button onClick={() => setSimSelBack(-1)} style={{ width: 48, height: 64, flexShrink: 0, borderRadius: 8, background: C.surfaceAlt, color: sel === -1 ? C.violet : C.faint, border: `2px solid ${sel === -1 ? C.violet : "transparent"}`, fontSize: 11, cursor: "pointer" }}>none</button>}
                      {list.map(({ p, i }) => (
                  ...
                  <div>
                    {rows.map(([lbl, src, shot]) => (
                      <div key={lbl} style={{ marginBottom: 10 }}>
                        <div style={{ fontSize: 11.5, color: C.faint, marginBottom: 4, fontWeight: 700, letterSpacing: 0.6 }}>{lbl.toUpperCase()}</div>
                        <div style={{ display: "flex", gap: 10 }}>
                          <div style={{ flex: 1 }}>
                            <div style={{ fontSize: 12, color: C.muted, marginBottom: 5, fontWeight: 600 }}>Now</div>
                          ...
                          <div style={{ flex: 1 }}>
                            <div style={{ fontSize: 12, color: C.violet, marginBottom: 5, fontWeight: 600 }}>At goal · AI preview</div>
                            <div style={{ position: "relative", aspectRatio: "3/4", borderRadius: 12, overflow: "hidden", background: C.surfaceAlt, border: `1.5px solid ${C.violet}66` }}>
                              {shot ? <img src={shot.url} alt="Goal" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <div style={{ height: "100%", display: "flex", alignItems: "center", justifyContent: "center", color: C.faint, fontSize: 12, padding: 12, textAlign: "center" }}>{simBusy ? "Forecasting… ≈15s" : "No forecast yet"}</div>}
                              {shot && <button onClick={() => deleteSim(simShots.indexOf(shot))} style={{ position: "absolute", top: 6, right: 6, width: 26, height: 26, borderRadius: 13, background: "rgba(14,20,26,0.72)", color: "#fff", border: "none", fontSize: 14, cursor: "pointer" }}>✕</button>}
                            </div>
                          </div>
                        </div>
                      </div>
                    ))}
                    {srcB && lf && lb && !paired && <div style={{ fontSize: 12, color: C.caution, marginBottom: 8, lineHeight: 1.4 }}>These two were forecast separately, not as a set — re-forecast to render them together.</div>}
                    ...
                    {lf && srcF && <button onClick={() => shareSim(srcF, lf)} style={{ width: "100%", marginTop: 8, background: "none", color: C.violet, border: `1.5px solid ${C.violet}`, borderRadius: 11, padding: "12px 0", fontFamily: BODY, fontSize: 14.5, fontWeight: 700, cursor: "pointer" }}>Share Now vs Goal →</button>}
                    <div style={{ fontSize: 11, color: C.faint, marginTop: 8, lineHeight: 1.45 }}>Same person, same clothing — only body composition changes, computed from your logged weight vs goal. A visualization, not a prediction. Gemini key required · ~7¢ per render{srcB ? ", two renders per set" : ""}.</div>
                  </div>
                ); })()}
            </div>
          </div>
          ...
          <div onClick={() => setSettingsOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 60, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
            <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 430, background: C.surface, borderRadius: "22px 22px 0 0", padding: "20px 20px calc(28px + env(safe-area-inset-bottom, 0px))", maxHeight: "82vh", overflowY: "auto" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
                <div style={{ fontFamily: DISPLAY, fontSize: 21, fontWeight: 700, color: C.ink }}>Settings</div>
                <button onClick={() => setSettingsOpen(false)} style={{ background: C.surfaceAlt, border: "none", width: 30, height: 30, borderRadius: 99, color: C.muted, fontSize: 16, cursor: "pointer" }}>✕</button>
              </div>
                ...
                {Object.entries(MODES).map(([k, m]) => {
                  ...
                  return (
                    <button key={k} onClick={() => pickMode(k)} style={{ textAlign: "left", padding: "12px 14px", borderRadius: 12, cursor: "pointer", border: `1.5px solid ${on ? C.go : C.hair}`, background: on ? C.goSoft : C.surface }}>
                      <div style={{ fontSize: 15, fontWeight: 700, color: on ? C.go : C.ink }}>{m.label}</div>
                      <div style={{ fontSize: 12.5, color: C.muted, marginTop: 2 }}>{m.targets.protein}g P · {m.targets.calories} cal</div>
                    </button>
                  );
                })}
              </div>
              ...
              <div style={{ fontSize: 12.5, color: C.muted, marginTop: -4, marginBottom: 10, lineHeight: 1.4 }}>Tap to flag. ForkCaster will never suggest a food that contains these.</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 20 }}>
                {ALLERGENS.map((a) => {
                  const on = allergies.includes(a);
                  return (
                    <button key={a} onClick={() => toggleIn(allergies, setAllergies, a)} style={{ padding: "8px 13px", borderRadius: 20, cursor: "pointer", fontFamily: DATA, fontSize: 11.5, fontWeight: 700, letterSpacing: 0.7, textTransform: "uppercase", border: `1px solid ${on ? C.avoid : C.hair}`, background: on ? C.avoid : C.surface, color: on ? "#fff" : C.muted, display: "flex", alignItems: "center", gap: 6 }}>
                      {on && <span style={{ fontSize: 13 }}>✕</span>}{a}
                    </button>
                  );
                })}
                {allergies.filter((a) => !ALLERGENS.includes(a)).map((a) => (
                  <button key={a} onClick={() => toggleIn(allergies, setAllergies, a)} style={{ padding: "8px 13px", borderRadius: 20, cursor: "pointer", fontFamily: DATA, fontSize: 11.5, fontWeight: 700, letterSpacing: 0.7, textTransform: "uppercase", border: `1px solid ${C.avoid}`, background: C.avoid, color: "#fff", display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 20, marginTop: -10 }}>
                <input value={customAllergy} onChange={(e) => setCustomAllergy(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && customAllergy.trim()) { toggleIn(allergies, setAllergies, customAllergy.trim()); setCustomAllergy(""); } }} placeholder="Add another allergy (e.g. mustard, avocado)"
                  style={{ flex: 1, fontFamily: BODY, fontSize: 14, color: C.ink, background: C.surfaceAlt, border: `1px solid ${C.hair}`, borderRadius: 10, padding: "10px 12px", outline: "none" }} />
                <button onClick={() => { if (customAllergy.trim()) { toggleIn(allergies, setAllergies, customAllergy.trim()); setCustomAllergy(""); } }} style={{ background: "transparent", color: C.avoid, border: `1px solid ${C.avoid}66`, borderRadius: 999, padding: "0 15px", fontFamily: DATA, fontSize: 11.5, fontWeight: 800, letterSpacing: 1, textTransform: "uppercase", cursor: "pointer" }}>Add</button>
              </div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 8 }}>
                {DIETS.map((d) => {
                  ...
                  return (
                    <button key={d} onClick={() => toggleIn(diets, setDiets, d)} style={{ padding: "8px 13px", borderRadius: 20, cursor: "pointer", fontFamily: DATA, fontSize: 11.5, fontWeight: 700, letterSpacing: 0.7, textTransform: "uppercase", border: `1px solid ${on ? C.go : C.hair}`, background: on ? C.go : C.surface, color: on ? "#fff" : C.muted }}>{d}</button>
                  );
                })}
              </div>
              <div style={{ fontSize: 12, color: C.faint, marginTop: 12, lineHeight: 1.4 }}>Switching goal mode resets today's targets to that preset. Allergy filtering applies to every meal suggestion and the coach.</div>
                ...
                {(() => {
                  const row = (label, control) => (
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 0", borderBottom: `1px solid ${C.hair}` }}>
                      <div style={{ fontSize: 14, color: C.ink2, paddingRight: 10 }}>{label}</div>
                      <div style={{ flexShrink: 0 }}>{control}</div>
                    </div>
                  );
                  const sel = (val, opts, on) => (
                    <select value={val} onChange={(e) => on(e.target.value)} style={{ background: "transparent", color: C.muted, border: `1px solid ${C.hair}`, borderRadius: 999, padding: "5px 11px", fontFamily: DATA, fontSize: 11, fontWeight: 700, letterSpacing: 0.9, textTransform: "uppercase", WebkitAppearance: "none", appearance: "none", textAlign: "center" }}>
                  ...
                  return (
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 700, color: C.faint, letterSpacing: 0.8, textTransform: "uppercase", margin: "6px 0 2px" }}>Day & units</div>
                      ...
                      <div style={{ fontSize: 12, fontWeight: 700, color: C.faint, letterSpacing: 0.8, textTransform: "uppercase", margin: "14px 0 2px" }}>GLP-1</div>
                      ...
                      <div style={{ fontSize: 12, fontWeight: 700, color: C.faint, letterSpacing: 0.8, textTransform: "uppercase", margin: "14px 0 2px" }}>Map & nearby</div>
                      ...
                      <div style={{ fontSize: 12, fontWeight: 700, color: C.faint, letterSpacing: 0.8, textTransform: "uppercase", margin: "14px 0 2px" }}>AI</div>
                      ...
                      <div style={{ fontSize: 12, color: C.faint, marginTop: 10, lineHeight: 1.4 }}>Changes save automatically and apply immediately. Haiku costs ~10x less per coach chat and venue ranking.</div>
                    </div>
                  );
                })()}
              </div>
              ...
              <div style={{ marginTop: 22, paddingTop: 16, borderTop: `1px solid ${C.hair}` }}>
                ...
                <div style={{ fontSize: 12.5, color: C.muted, marginTop: -4, marginBottom: 10, lineHeight: 1.45 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={saveKeys} disabled={!Object.values(keyIn).some((v) => String(v || "").trim())} style={{ flex: 1, background: C.go, color: C.bg, border: "none", borderRadius: 999, padding: "12px 0", fontFamily: DATA, fontSize: 12.5, fontWeight: 800, letterSpacing: 1.1, textTransform: "uppercase", cursor: "pointer", opacity: !Object.values(keyIn).some((v) => String(v || "").trim()) ? 0.5 : 1 }}>Save keys</button>
                  <button onClick={testAiKey} style={{ flex: 1, background: "none", color: C.ink, border: `1.5px solid ${C.hair}`, borderRadius: 10, padding: "11px 0", fontFamily: DATA, fontSize: 12.5, fontWeight: 800, letterSpacing: 1, textTransform: "uppercase", cursor: "pointer" }}>Test AI key</button>
                </div>
              ...
              <div style={{ marginTop: 22, paddingTop: 16, borderTop: `1px solid ${C.hair}` }}>
                ...
                <button onClick={togglePush} disabled={pushBusy} style={{ width: "100%", background: pushOn ? C.go : "none", color: pushOn ? C.surface : C.go, border: `1.5px solid ${C.go}`, borderRadius: 11, padding: "12px 0", fontFamily: DATA, fontSize: 12.5, fontWeight: 800, letterSpacing: 1, textTransform: "uppercase", cursor: "pointer", marginBottom: 6, opacity: pushBusy ? 0.6 : 1 }}>{pushOn ? "✓ Dose-day push reminders ON" : "Enable dose-day push reminders"}</button>
                <div style={{ fontSize: 11.5, color: C.faint, marginBottom: 16, lineHeight: 1.45 }}>Fires at your reminder hour on dose day with the suggested site. iPhone: add ForkCaster to the Home Screen first (Share → Add to Home Screen).</div>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                  <button onClick={exportJSON} style={{ flex: 1, background: "none", color: C.ink2, border: `1.5px solid ${C.hair}`, borderRadius: 11, padding: "11px 0", fontFamily: DATA, fontSize: 12, fontWeight: 800, letterSpacing: 1, textTransform: "uppercase", cursor: "pointer" }}>Backup (JSON)</button>
                  <label style={{ flex: 1, background: "none", color: C.ink2, border: `1.5px solid ${C.hair}`, borderRadius: 11, padding: "11px 0", fontFamily: DATA, fontSize: 12, fontWeight: 800, letterSpacing: 1, textTransform: "uppercase", cursor: "pointer", textAlign: "center" }}>Restore…<input type="file" accept=".json,application/json" style={{ display: "none" }} onChange={(e) => { if (e.target.files && e.target.files[0]) restoreJSON(e.target.files[0]); e.target.value = ""; }} /></label>
                </div>
                <div style={{ fontSize: 11.5, color: C.faint, marginBottom: 16, lineHeight: 1.45 }}>The PDF is a clean, organized report (doses &amp; sites, weight, side effects, nutrition) you can email or AirDrop to your care team. JSON is the full-fidelity backup for restore.</div>
                <div style={{ marginBottom: 12, background: C.surfaceAlt, borderRadius: 11, padding: "11px 12px" }}>
                  <div style={{ fontSize: 13.5, color: C.ink, fontWeight: 700 }}>Restore a previous save</div>
                  <div style={{ fontSize: 12, color: C.faint, marginTop: 3, lineHeight: 1.45 }}>Your node keeps recent snapshots of your data. If a save ever comes back emptier than it should, restore the one before it.</div>
                  <button onClick={async () => { try { const d = await fetch("/api/state/backups").then((r) => r.json()); setBackups(d.backups || []); } catch { setBackups([]); } }}
                    style={{ marginTop: 8, background: "none", border: `1.5px solid ${C.hair}`, color: C.ink, borderRadius: 9, padding: "8px 12px", fontFamily: DATA, fontSize: 11, fontWeight: 800, letterSpacing: 0.9, textTransform: "uppercase", cursor: "pointer" }}>Show snapshots</button>
                  {backups && backups.map((b) => (
                    <div key={b.file} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, padding: "8px 0", borderTop: `1px solid ${C.hair}`, marginTop: 8 }}>
                      <div>
                        <div style={{ fontSize: 13, color: C.ink, fontWeight: 700 }}>{b.when.replace("T", " ")}{b.reason === "shrink" ? <span style={{ color: C.caution }}> · save shrank</span> : ""}</div>
                        <div style={{ fontSize: 11.5, color: C.muted }}>{b.counts.meals} meals · {b.counts.weights} weigh-ins · {b.counts.workouts} workouts · {b.counts.doses} doses</div>
                      </div>
                      <button onClick={async () => { if (!window.confirm(`Restore this snapshot? Everything logged since then is replaced. A copy of your current data is kept first.`)) return;
                        ...
                        style={{ background: C.surface, border: `1.5px solid ${C.hair}`, color: C.ink, borderRadius: 9, padding: "7px 11px", fontFamily: DATA, fontSize: 10.5, fontWeight: 800, letterSpacing: 0.8, textTransform: "uppercase", cursor: "pointer", whiteSpace: "nowrap" }}>Restore</button>
                    </div>))}
                  {backups && backups.length === 0 && <div style={{ fontSize: 12, color: C.faint, marginTop: 8 }}>No snapshots yet — one is kept the first time your data changes after an update.</div>}
                </div>
                {prefs.hideHealthCard && <button onClick={() => setPrefs({ ...prefs, hideHealthCard: false })} style={{ width: "100%", marginBottom: 12, background: C.surfaceAlt, border: `1.5px solid ${C.hair}`, color: C.ink, borderRadius: 11, padding: "11px 0", fontFamily: DATA, fontSize: 11.5, fontWeight: 800, letterSpacing: 0.9, textTransform: "uppercase", cursor: "pointer" }}>Show the Apple Health card again</button>}
                <div style={{ marginBottom: 12, background: C.surfaceAlt, borderRadius: 11, padding: "11px 12px" }}>
                  <div style={{ fontSize: 13.5, color: C.ink, fontWeight: 700 }}>Images on your node</div>
                  ...
                  }} style={{ marginTop: 8, background: "none", border: `1.5px solid ${C.hair}`, color: C.ink, borderRadius: 9, padding: "8px 12px", fontFamily: DATA, fontSize: 11, fontWeight: 800, letterSpacing: 0.9, textTransform: "uppercase", cursor: "pointer" }}>Sweep orphaned images</button>
                  <div style={{ fontSize: 11.5, color: C.faint, marginTop: 6 }}>Deletes only images the app no longer references — photos, comparisons and forecasts in use are kept.</div>
                </div>
                <button onClick={async () => { if (window.confirm("Reset ALL ForkCaster data on your node? Weight, meals, GLP-1 logs, settings — AND every progress photo and forecast image stored on the node. This cannot be undone.")) { hydrated.current = false; try { await fetch("/api/state?photos=1", { method: "DELETE" }); } catch {} window.location.reload(); } }} style={{ width: "100%", background: "none", color: C.avoid, border: `1.5px solid ${C.avoid}66`, borderRadius: 11, padding: "12px 0", fontFamily: DATA, fontSize: 12.5, fontWeight: 800, letterSpacing: 1, textTransform: "uppercase", cursor: "pointer" }}>Reset all data — start fresh</button>
              </div>
              <div style={{ textAlign: "center", fontSize: 12, color: C.faint, marginTop: 18 }}>ForkCaster {appVer ? `v${appVer}` : ""}</div>
            </div>
          </div>
        )}
      </div>
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6, gap: 8 }}>
        <div style={{ fontFamily: DATA, fontSize: 11.5, fontWeight: 700, color: C.muted, textTransform: "uppercase", letterSpacing: 1.6, whiteSpace: "nowrap" }}>Estimated med level</div>
        <div style={{ fontFamily: DATA, fontSize: 11.5, fontWeight: 700, letterSpacing: 1, color: C.violet, background: C.violet + "22", border: `1px solid ${C.violet}66`, borderRadius: 999, padding: "5px 12px", whiteSpace: "nowrap", flexShrink: 0 }}>{absorbing ? "ABSORBING" : climbing ? "CLIMBING" : "STEADY"} · {vsPeak}%</div>
      </div>
...
function SymptomPatterns({ C, sideEffects, doseLog }) {
  ...
  return (
    <div>
      <div style={{ fontSize: 12, color: C.muted, textTransform: "uppercase", letterSpacing: 0.4, marginBottom: 8 }}>Symptom patterns</div>
      {lines.map((l) => <div key={l.sym} style={{ fontSize: 13.5, color: C.ink2, lineHeight: 1.5, marginBottom: 6 }}>{"\u2022"} {l.text}</div>)}
      <div style={{ fontSize: 11, color: C.faint, marginTop: 4 }}>Computed from your logs {"—"} worth mentioning to your prescriber if a pattern is disruptive.</div>
    </div>
  );
}
...
function WeeklyCard({ C, mealLog, weightLog, doseLog, sideEffects, proteinGoal, fmtW, unit, goalLbs, onShareMilestone }) {
  ...
  const kpi = (v, l, col) => (
    <div style={{ flex: 1, textAlign: "center", padding: "10px 4px", borderRadius: 12, background: C.surfaceAlt, border: `1px solid ${C.hair}` }}>
      <div style={{ fontFamily: DISPLAY, fontSize: 20, fontWeight: 700, color: col || C.ink }}>{v}</div>
      <div style={{ fontSize: 10.5, color: C.faint, marginTop: 2, lineHeight: 1.3 }}>{l}</div>
    </div>
  );
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
        <div style={{ fontFamily: DATA, fontSize: 11.5, fontWeight: 700, letterSpacing: 1.6, textTransform: "uppercase", color: C.muted }}>This week</div>
        {msHit != null && <span style={{ fontFamily: DATA, fontSize: 9.5, fontWeight: 700, letterSpacing: 1, borderRadius: 999, padding: "3px 9px", marginTop: -8, marginTop: 0, color: C.go, border: `1px solid ${C.go}55`, background: C.go + "1A" }}>MILESTONE · {msHit} {unit.toUpperCase()} DOWN</span>}
      </div>
      <div style={{ display: "flex", gap: 8 }}>
      ...
      <div style={{ fontSize: 11, color: C.faint, marginTop: 7 }}>{loggedDays}/7 days with food logged · adherence = average of daily protein vs goal</div>
      {(() => {
        const wl = weightLog || [];
        ...
        if (slope >= -0.01) return <div style={{ fontSize: 12, color: C.faint, marginTop: 6 }}>Goal ETA: needs a downward trend to project.</div>;
        const daysTo = Math.round((cur - goalLbs) / -slope);
        if (daysTo <= 0) return <div style={{ fontSize: 12.5, color: C.go, marginTop: 6, fontWeight: 700 }}>You’re at (or past) goal — maintenance mode.</div>;
        if (daysTo > 900) return null;
        const eta = new Date(); eta.setDate(eta.getDate() + daysTo);
        return <div style={{ fontSize: 12.5, color: C.ink2, marginTop: 6 }}><b style={{ color: C.go }}>{fmtW(goalLbs, 0)} {unit} ≈ {eta.toLocaleDateString("en-US", { month: "short", day: "numeric", year: eta.getFullYear() !== new Date().getFullYear() ? "numeric" : undefined })}</b> at your recent pace · pace varies, this will shift</div>;
      })()}
      {(() => {
        const wl = weightLog || [];
        ...
        if (!hit) return null;
        return <button onClick={() => onShareMilestone && onShareMilestone(hit, lost)} style={{ width: "100%", marginTop: 10, background: "none", color: C.go, border: `1.5px solid ${C.go}`, borderRadius: 11, padding: "11px 0", fontFamily: BODY, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>Share your −{hit} {unit} milestone →</button>;
      })()}
    </div>
  );
}
...
export function SiteAvatar({ C, sex, bmi, doseLog, perSite, pendingSite, setPendingSite, mini }) {
  ...
  return (
    <div>
      <div style={{ fontFamily: DATA, fontSize: 11.5, fontWeight: 700, letterSpacing: 1.6, textTransform: "uppercase",  fontSize: 12, color: C.muted, textTransform: "uppercase", letterSpacing: 0.4, marginBottom: 4 }}>
      ...
      <div style={{ fontSize: 11.5, color: C.faint, marginBottom: 6 }}>Cycle {cycleLogged}/{cyc} · {perSite} per site · resets when every site is used</div>
      ...
      {suggested && !pendingSite && (<div style={{ marginTop: 8 }}><span style={{ fontFamily: DATA, fontSize: 9.5, fontWeight: 700, letterSpacing: 1, borderRadius: 999, padding: "3px 9px", color: C.violet, border: `1px solid ${C.violet}55`, background: C.violet + "1A", textTransform: "uppercase" }}>NEXT: {suggested}</span></div>)}
      <div style={{ fontSize: 11, color: C.faint, marginTop: 8, lineHeight: 1.5 }}>L/R are <b>your</b> left and right (front view, so they appear mirrored — same as the R marker on an X-ray). Arm dots mean the <b>back</b> of the upper arm. Tap a site, then Log dose — it saves with the dose. Faded sites are used up this cycle.</div>
    </div>
  );
}
...
const pillIc = (color, s = 12) => (
  ...
  const navB = { background: "none", border: `1px solid ${C.hair}`, borderRadius: 8, color: C.ink2, width: 28, height: 28, fontSize: 16, cursor: "pointer", lineHeight: 1 };
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
        <button onClick={() => setYm(({ y, m }) => (m === 0 ? { y: y - 1, m: 11 } : { y, m: m - 1 }))} style={navB}>‹</button>
        <div style={{ fontFamily: DATA, fontSize: 11.5, fontWeight: 700, letterSpacing: 1.4, textTransform: "uppercase", color: C.muted }}>Dose calendar · {monthName}</div>
        <button onClick={() => setYm(({ y, m }) => (m === 11 ? { y: y + 1, m: 0 } : { y, m: m + 1 }))} style={navB}>›</button>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", marginBottom: 4 }}>
        {["S", "M", "T", "W", "T", "F", "S"].map((d, i) => <div key={i} style={{ textAlign: "center", fontFamily: DATA, fontSize: 10, fontWeight: 700, letterSpacing: 1, color: C.faint }}>{d}</div>)}
      </div>
        ...
        {cells.map((d, i) => {
          ...
          return (
            <div key={i} onClick={() => { if (lg && onRemove) onRemove(di); }} style={{ height: 36, cursor: lg ? "pointer" : "default", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 1, borderRadius: 9, margin: "0 2px", border: due ? `1.5px dashed ${C.violet}` : today ? `1.5px solid ${C.go}88` : "1.5px solid transparent", background: lg ? C.goSoft : "transparent" }}>
              <span style={{ fontSize: 11.5, fontWeight: today || due || lg ? 800 : 500, color: lg ? C.go : due ? C.violet : today ? C.ink : C.muted }}>{d}</span>
              {lg ? (pill ? pillIc(C.go) : syr(C.go)) : due ? <span style={{ fontSize: 9.5, color: C.violet, fontWeight: 800, letterSpacing: 0.5 }}>DUE</span> : <span style={{ height: 11 }} />}
            </div>
          );
        })}
      </div>
      <div style={{ display: "flex", gap: 14, justifyContent: "center", marginTop: 10, fontSize: 11, color: C.faint, alignItems: "center" }}>
    ...
    <input type="text" inputMode="decimal" value={draft} step={step}
      ...
      style={bare
        ? { width: w || 64, background: "transparent",
            ...
            fontFamily: "ui-monospace, monospace", fontSize: 11, fontWeight: 700, letterSpacing: 0.6,
            textAlign: "center", outline: "none", boxSizing: "border-box"}
        : { width: "100%", boxSizing: "border-box", fontFamily: DISPLAY, fontSize: 18, fontWeight: 600, color: C.ink, background: C.surfaceAlt, border: `1px solid ${C.hair}`, borderRadius: 10, padding: "9px 11px", outline: "none" }} />
  );
  if (bare) return input;
  return (<div style={{ flex: 1 }}><div style={{ fontSize: 12, color: C.muted, marginBottom: 4 }}>{label}</div>{input}</div>);
}
...
function MapView({ C, geo, restaurants, onPin, scoreColor, onSearchArea, prefs }) {
  ...
  return (
    <div style={{ position: "relative", height: 280, borderRadius: 16, overflow: "hidden", border: `1px solid ${C.hair}`, isolation: "isolate", zIndex: 0 }}>
      ...
      <div style={{ position: "absolute", top: 10, left: 10, zIndex: 800, background: pillBg, borderRadius: 20, padding: "4px 11px", fontSize: 12, fontWeight: 600, color: pillInk, display: "flex", gap: 5, alignItems: "center", pointerEvents: "none" }}>
      ...
      <div style={{ position: "absolute", top: 10, right: 10, zIndex: 800, display: "flex", gap: 4, background: pillBg, borderRadius: 20, padding: 3 }}>
        {["auto", "day", "night", "sat"].map((k) => (
          <button key={k} onClick={() => setPick(k)} style={{ border: "none", cursor: "pointer", borderRadius: 16, padding: "3px 9px", fontFamily: BODY, fontSize: 11.5, fontWeight: 700, background: pick === k ? C.go : "transparent", color: pick === k ? "#fff" : pillInk, textTransform: "capitalize" }}>{k}</button>
        ))}
      </div>
      {moved && (
        <button onClick={() => { const c = mapRef.current && mapRef.current.getCenter(); if (c && onSearchArea) { onSearchArea(c.lat, c.lng); } setMoved(false); }}
          style={{ position: "absolute", bottom: 12, left: "50%", transform: "translateX(-50%)", zIndex: 800, background: C.go, color: "#fff", border: "none", borderRadius: 20, padding: "9px 16px", fontFamily: BODY, fontSize: 13.5, fontWeight: 700, cursor: "pointer", boxShadow: "0 3px 10px rgba(0,0,0,0.35)" }}>
          Search this area
        </button>
      )}
      ...
      {insecure && !geo.live && (
        <div style={{ position: "absolute", bottom: 12, left: 10, right: 10, zIndex: 800, background: "rgba(200,140,20,0.92)", color: "#1a1200", borderRadius: 10, padding: "6px 10px", fontSize: 12, fontWeight: 600, textAlign: "center" }}>
