      ...
      <div>
        ...
        {card(<div>
          ...
          {(() => { const est = groceryEst;
            if (!est.matched) return (<div style={{ fontSize: 11, color: C.faint, marginTop: 9, lineHeight: 1.45 }}>No cost estimate yet — prices you save in Shop Mode build a price book per store, and this totals only what you have actually paid.</div>);
            return (<div style={{ marginTop: 9 }}>
              {est.stores.map((st) => (
                <div key={st.store} style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 8, padding: "3px 0" }}>
                  <span style={{ fontSize: 12.5, color: C.ink, fontWeight: 700, textTransform: "capitalize" }}>{st.store}{est.cheapest && est.cheapest.store === st.store ? <span style={{ color: C.go, fontWeight: 700 }}> · cheapest</span> : ""}</span>
                  <span style={{ fontSize: 13, color: C.ink, fontWeight: 800, whiteSpace: "nowrap" }}>≈ ${st.total.toFixed(2)} <span style={{ fontSize: 11, color: C.muted, fontWeight: 500 }}>· {st.matched}/{mealPlan.grocery.length}</span></span>
                </div>))}
  ...
  return (
    <div>
      ...
      {(() => {
        ...
        if (slope >= -0.01) return <div style={{ fontSize: 11, color: C.faint, marginTop: 6 }}>Goal ETA: needs a downward trend to project.</div>;
        const daysTo = Math.round((cur - goalLbs) / -slope);
        if (daysTo <= 0) return <div style={{ fontSize: 11.5, color: C.go, marginTop: 6, fontWeight: 700 }}>You’re at (or past) goal — maintenance mode.</div>;
        if (daysTo > 900) return null;
        const eta = new Date(); eta.setDate(eta.getDate() + daysTo);
        return <div style={{ fontSize: 11.5, color: C.ink2, marginTop: 6 }}><b style={{ color: C.go }}>{fmtW(goalLbs, 0)} {unit} ≈ {eta.toLocaleDateString("en-US", { month: "short", day: "numeric", year: eta.getFullYear() !== new Date().getFullYear() ? "numeric" : undefined })}</b> at your recent pace · pace varies, this will shift</div>;
      })()}
      {(() => {
        const wl = weightLog || [];
        ...
        if (!hit) return null;
        return <button onClick={() => onShareMilestone && onShareMilestone(hit, lost)} style={{ width: "100%", marginTop: 10, background: "none", color: C.go, border: `1.5px solid ${C.go}`, borderRadius: 11, padding: "11px 0", fontFamily: BODY, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Share your −{hit} {unit} milestone →</button>;
      })()}
    </div>
  );
}
